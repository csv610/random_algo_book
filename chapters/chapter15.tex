...
Given the factorization $n=pq$, one can solve the RSA inversion problem in
polynomial time by computing $d=e^{-1}\bmod\lambda(n)$ and returning
$c^d\bmod n$.
\end{theorem}
...
By the inductive hypothesis, the probability that $g_k$ vanishes on the
first $n-1$ random coordinates is at most $(d-k)/|S|$.  When it does not
vanish, the specialized polynomial has degree exactly $k$ and therefore
has at most $k$ roots in $S$.  Hence
\[
\Pr[f(r_1,\ldots,r_n)=0]
...
\begin{proof}
For prime $n$, Fermat's theorem implies that the sequence
$a^d,a^{2d},\ldots,a^{2^sd}=1$ either starts at $1$ or reaches $1$ through
$-1$, because a field has only the two square roots of $1$. Thus every
coprime base passes. For composite $n$, the standard strong-liar lemma uses
the Chinese remainder theorem and the square-root structure of the composite
residue ring to bound the passing bases by one quarter of the reduced residue
system. This is the nontrivial group-theoretic part of the theorem; the exact
sampling convention and the small-input cases are stated explicitly above.
\end{proof}
...
and contradict compositeness. Equivalently, the witnesses contain a coset of
a proper subgroup of the unit group. Therefore at least half of the units are
witnesses. This is the exact group-theoretic lemma used by the test; the
incorrect shortcut that the full liar set is automatically a subgroup is not
needed.
\end{proof}
