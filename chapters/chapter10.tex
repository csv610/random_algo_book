...
that no three input points are collinear and no four are cocircular; these
assumptions make triangulations and their local flips unambiguous. For
degenerate inputs, a symbolic perturbation or a deterministic tie-breaking
rule must be specified, and statements involving uniqueness or strict
inequalities should not be read as applying without that convention.
...
\begin{theorem}[Linear-time diameter after the hull]
\label{thm:diameter-rand}
After the convex hull has been computed, the diameter of $n$ points in
$\mathbb{R}^2$ can be computed in deterministic $O(n)$ time.
\end{theorem}

\begin{proof}
The diameter of a planar point set is realized by two convex-hull vertices.
Apply the rotating-calipers algorithm to the cyclically ordered hull vertices.
Each caliper advances at most once around the hull, so the number of
antipodal pairs examined is linear in the hull size and hence $O(n)$.
\end{proof}
...
\begin{proof}
This is the standard VC-dimension $\varepsilon$-net theorem. The proof uses
double sampling: a random sample that misses a range of measure at least
$\varepsilon$ is witnessed, with constant probability, by a discrepancy
between two independent samples. Sauer's lemma bounds the number of traces of
the range space on the combined sample by
$O((s/d)^d)$, where $s$ is the sample size. A union bound over these traces
gives failure probability at most $\delta$ when
$s=O(\varepsilon^{-1}(d\log(1/\varepsilon)+\log(1/\delta)))$.
\end{proof}
...
\]
For fixed $d$, an induction on $d$ gives $T(n,d)=O(c_d n)$.
Substituting the inductive bound for dimension $d-1$ and summing the
resulting $O(d c_{d-1}/n)$ contribution over all recursion levels gives
$c_d=O(d c_{d-1}+d)$, with $c_0=O(1)$.  Thus $c_d=O(d!)$ and
$T(n,d)=O(d!\,n)$.
\end{proof}
