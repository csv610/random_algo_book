...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\textit{9. Data Structures}}
...
\addcontentsline{toc}{chapter}{9 Data Structures}
\markboth{9 Data Structures}{Randomized Algorithms}
...
A standard affine family is obtained by choosing $a\neq0$ and $b$ uniformly
modulo a prime $p>|\mathcal U|$ and defining the intermediate hash
$g_{a,b}(x)=(ax+b)\bmod p$.  Mapping this value to $m$ slots by quotienting
can introduce a rounding factor; the resulting family is $2/m$-universal in
general, not exactly $1/m$-universal.  Exact $1/m$ universality requires a
construction whose range has the desired size (or a standard rejection step).
The family $\mathcal{H}$ is therefore suitable for expected constant-time
chaining when $m=\Theta(n)$, with the constant adjusted for the $2/m$
collision bound.
...
Standard dynamic perfect-hashing schemes use incremental rebuilding; a
full-table rebuild after a load change does not by itself give a worst-case
update bound.

\begin{theorem}\label{thm:dynamic-perfect}
There are dynamic perfect-hashing schemes with $O(1)$ worst-case search,
$O(1)$ amortized expected update time, and $O(n)$ space, provided that
rebuilding is scheduled incrementally.
\end{theorem}

\begin{proof}
The claim is not proved by a full-table rebuild: that rebuild can take
$\Theta(n)$ time.  The standard construction maintains an old and a new
table simultaneously, performs a constant amount of rebuilding work at each
update, and switches tables only after the new structure is complete.  The
FKS space bound applies to each completed table, while load-factor
thresholds ensure that the rebuilding work finishes before another rebuild
is required.  The details depend on the particular dynamic perfect-hashing
construction.
\end{proof}
