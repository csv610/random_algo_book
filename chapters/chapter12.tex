...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\textit{12. Approximate Counting}}
...
\chapter*{12 Approximate Counting}
\addcontentsline{toc}{chapter}{12 Approximate Counting}
\markboth{12 Approximate Counting}{Randomized Algorithms}
...
samplable conditional distributions yields an FPRAS by estimating the
successive ratios. Without these conditions, no general equivalence is
claimed.
\end{proposition}
...
estimated ratio. A union bound over the polynomially many levels controls
the accumulated total-variation error. The reverse direction is not obtained
by a coupon-collector argument: that would require time proportional to
$|\Omega|$. Instead, the assumed overlapping sequence makes each ratio a
bounded-variance importance-sampling estimate, and the product of the estimates
recovers the total count. The overlap and error-allocation hypotheses are
essential.
\end{proof}
...
\begin{proof}
Let $Y_0$ be drawn from $\pi$. Then $Y_t$ has distribution $\pi$ for all $t$ (since $\pi$ is stationary). By the coupling inequality, the total variation distance between the two marginal distributions is at most $\Pr[X_t\neq Y_t]$. The total variation distance is:
\[
...
\Function{DNFCount}{$\phi = C_1 \lor \cdots \lor C_m$, error $\varepsilon$, confidence $\delta$}
    ...
    \State $N \gets \lceil \frac{2m^2}{\varepsilon^2\delta} \rceil$ \Comment{number of samples}
    \For{$i = 1$ \textbf{to} $N$}
    ...
    \State Set estimator $\hat{Y}_i \gets Z / C(\sigma_i)$
    \EndFor
...
\]
in running time $O\!\left(\frac{m^2 n}{\varepsilon^2\delta}\right)$ for the
straightforward implementation.
\end{theorem}

\begin{proof}
The sampling distribution satisfies
\[
 \Pr[\sigma]=\frac{\rho(\sigma)}{Z},
 \qquad \rho(\sigma)=|\{j:\sigma\in A_j\}|,
\]
for every satisfying assignment.  Therefore the estimator
$Y=Z/\rho(\sigma)$ is unbiased:
\[
 \mathbb{E}[Y]=\sum_{\sigma\in\cup_j A_j}
 \frac{Z}{\rho(\sigma)}\frac{\rho(\sigma)}{Z}
 =|\cup_j A_j|=\operatorname{sat}(\phi).
\]
Moreover, $Z/m\leq Y\leq Z$ and $Z\leq m\operatorname{sat}(\phi)$.
Thus $\operatorname{Var}(Y)\leq m^2\operatorname{sat}(\phi)^2$.
Chebyshev's inequality gives failure probability at most
$m^2/(N\varepsilon^2)$.  Taking
$N=\lceil m^2/(\varepsilon^2\delta)\rceil$ proves the stated FPRAS
bound (with the straightforward implementation running in
$O(mnN)$ time).
...
\textbf{Running time:} Computing $Z$ takes $O(m)$ time. Each sample takes $O(m+n)$ time to generate and inspect the assignment.
\end{proof}
\fi
\end{proof}
...
\Function{DNFCount}{$\phi = C_1 \lor \cdots \lor C_m$, error $\varepsilon$}
    ...
    \State $Z \gets \sum_{j=1}^m |A_j|$
    \State $N \gets \lceil 2m^2 / \varepsilon^2 \rceil$
    \For{$i = 1$ \textbf{to} $N$}
...
\label{thm:karpluby2}
Algorithm~\ref{alg:karpluby-correct} is an FPRAS for DNF counting. With the
straightforward clause scan, it runs in time
$O(mn\,m^2/\varepsilon^2)$ and outputs $\hat{Y}$ satisfying
\[
...
\begin{proof}
The sampling distribution is $\Pr[\sigma]=\rho(\sigma)/Z$, so
$\mathbb{E}[\hat{Y}_i]=\operatorname{sat}(\phi)$. Also
$0\leq\hat{Y}_i\leq Z\leq m\operatorname{sat}(\phi)$. Hence
$\operatorname{Var}(\hat{Y}_i)\leq m^2\operatorname{sat}(\phi)^2$, and the
variance of the mean of $N$ independent samples is smaller by a factor $N$.
...
\]
With $N=4m^2/\varepsilon^2$, the failure probability is at most $1/4$.
Generating an assignment costs $O(n)$ and scanning all clauses costs $O(mn)$
under the direct representation, giving the stated runtime. The analysis makes
no claim of the sharper implementation bound sometimes obtained with
additional indexing of clauses.
\end{proof}
...
\label{def:jsv-chain}
The illustrative state space is $\Omega = M_n \cup M_{n-1}$. The transitions
below describe the underlying adjacency relation; the actual chain must add
laziness and the normalization factors required for detailed balance:
\begin{enumerate}[nosep]
...
The transition probabilities above are only schematic. They are not symmetric
when the numbers of neighboring matchings differ, so uniformity does not
follow from the displayed moves. The Jerrum--Sinclair--Vigoda chain uses
carefully normalized transition probabilities and a weighted state space to
obtain the required stationary distribution. Those factors must be included
in a complete proof or implementation.
\end{remark}

The correctly normalized JSV chain satisfies detailed balance with respect to
its explicitly defined weighted stationary distribution; the schematic chain
above is not, by itself, a proof of that fact.
...
\begin{proof}[Proof sketch]
For a reversible chain, the conductance is
\[
...
The technical JSV theorem supplies a polynomial lower bound on conductance for
each weighted chain. Its proof uses canonical paths and alternating-path
structure; it is not obtained by applying a state-space-size bound to the
naive chain. The conductance theorem then gives polynomial burn-in because
$\pi_{\min}$ and the conductance bound are controlled by the encoded input,
not by enumerating the exponentially large state space. An annealing sequence
with polynomial overlap estimates successive ratios, and a union bound over
its levels gives the stated FPRAS. The complete canonical-path construction
is cited as the JSV result rather than reproduced here.
\end{proof}
...
There is an FPRAS for the permanent of non-negative matrices. Given an
$n \times n$ non-negative matrix $A$ and an error parameter $\varepsilon$,
the algorithm computes $\hat{P}$ satisfying
\[
\Pr\!\bigl[(1 - \varepsilon) \operatorname{perm}(A) \leq \hat{P} \leq (1 + \varepsilon) \operatorname{perm}(A)\bigr] \geq \frac{3}{4}
\]
in time polynomial in the encoded input length and $1/\varepsilon$.
\end{theorem}

\begin{proof}
The proof is the Jerrum--Sinclair--Vigoda theorem. Its input contract is a
non-negative matrix represented with polynomially bounded precision. The
algorithm constructs an annealing sequence of weighted matching distributions;
each adjacent ratio is estimated by a lazy, rapidly mixing chain. If the $i$th
ratio is estimated within relative error $\eta_i$ and the estimates all
succeed, then the product has relative error at most
$(\prod_i(1+\eta_i))-1$ on the upper side and the analogous lower-side product.
Choosing $\eta_i=\Theta(\varepsilon/L)$ for $L$ levels and assigning failure
probability $\delta/L$ to each estimate gives total error at most
$\varepsilon$ and failure at most $\delta$. Polynomial overlap and mixing
bounds make the total running time polynomial. The canonical-path and
annealing proofs are part of the cited result and are not replaced by the
naive unweighted-chain argument.
\end{proof}
