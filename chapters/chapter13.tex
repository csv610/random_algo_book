...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\textit{13. Parallel and Distributed Algorithms}}
...
\chapter*{13 Parallel and Distributed Algorithms}
\addcontentsline{toc}{chapter}{13 Parallel and Distributed Algorithms}
\markboth{13 Parallel and Distributed Algorithms}{Randomized Algorithms}
\label{ch:parallel}
...
\fi
The standard Isolating Lemma states that independent weights from
$\{1,\ldots,2n\}$ isolate the minimum member of the family of perfect
matchings with probability at least $1/2$.  Its proof exposes weights one
edge at a time and charges each possible tie to a distinct critical
interval.  A union bound over all perfect matchings would be invalid because
their number may be exponential.
\end{proof}
...
is yes, and delete its endpoints. At most $n$ stages are needed, and the
oracle error is amplified so that the union of all false-positive probabilities
is at most the requested failure probability.

We do not replace the permanent by a randomly signed determinant: that
identity is false in general. Instead, assign independent random values to
the nonzero entries of the bipartite adjacency matrix and use the determinant
polynomial.
...
The stated polylogarithmic bound is the standard full parallel construction;
it does not follow from a determinant oracle plus sequential edge-by-edge
self-reduction alone.
\label{thm:rnc-pm}
...
\begin{proof}
The determinant oracle is only one component of the cited RNC construction.
The standard parallel isolation and self-reduction machinery supplies the
polylogarithmic depth; a naive sequential deletion would give polynomial
depth and is not being used to justify the theorem.
\end{proof}
...
identity test; a union bound handles the polynomially many tests. This is the
Karp--Vazirani--Vazirani construction cited below. We do not claim that the
permanent itself is available in $\mathbf{RNC}$; that would be an unsupported
step.
\end{proof}
...
\State Every processor broadcasts $c_i$ to all others.
\State Let $j$ be the smallest processor identifier.
\State Every processor outputs $c_j$.
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\label{thm:choice}
With at least one processor, the choice coordination algorithm achieves
consensus in one communication round. If the selected processor's value is
uniform on $\{1,\ldots,m\}$, the common output is uniform as well.
\end{theorem}

\begin{proof}
After the broadcast, every processor knows $c_j$ and therefore outputs the
same value. Since $c_j$ was sampled uniformly, the common output is uniform.
...
\begin{remark}[Limitation of the majority protocol]
\label{thm:random-byzantine}
The majority-of-random-bits pseudocode above is not a Byzantine agreement
protocol: it ignores the processors' input values, so it can violate validity
even when every honest processor starts with the same input. A correct
randomized protocol requires a specified message model, reliable broadcast or
equivalent consistency mechanism, and a common-coin or shared-randomness
primitive. Those assumptions must be stated before a correctness theorem is
claimed.
\iffalse
...
\begin{remark}
Randomized Byzantine agreement can be achieved under suitable synchronous
or asynchronous models, but the round and fault bounds depend on the exact
model and on the common-coin primitive. A complete protocol and proof should
be supplied before claiming an $O(1)$-round or high-probability guarantee.
\end{remark}
