...
\begin{proof}
The determinant polynomial is evaluated over a field of size larger than its
degree. Schwartz--Zippel gives one-sided false-zero probability at most
$n/|\mathbb F|$. Determinant evaluation is in $\mathbf{NC}^2$; repeating the
oracle with fresh field values makes the failure probability small. A matching
is then recovered by parallel self-reduction, invoking the oracle on the
graphs obtained by deleting candidate edges and their incident vertices. The
resulting depth is the oracle depth times the $O(n)$ self-reduction depth, so
the exact RNC bound depends on the chosen parallel self-reduction scheme; no
adjugate or minimum-weight dominance claim is needed.
\end{proof}
