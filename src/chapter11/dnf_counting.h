...
void print_dnf(const DNF& dnf) {
    ...
    for (const auto& clause : dnf.clauses) {
        if (!first_clause) std::print(" | ");
        first_clause = false;
        if (clause.literals.size() > 1) std::print("(");
        bool first_lit = true;
        for (const auto& lit : clause.literals) {
            if (!first_lit) std::print(" & ");
            first_lit = false;
            if (lit.is_negated) std::print("~");
            std::print("x{}", lit.variable + 1);
        }
        if (clause.literals.size() > 1) std::print(")");
    }
    std::println();
...
void demonstrate_dnf_counting() {
    ...
    // ---------------------------------------------------------------
    std::println("=== Part 1: Small DNF Formula -- Exact Enumeration ===\n");
    ...
    int n_small = 3;
    std::print("Formula (n={}): ", n_small);
    print_dnf(small_dnf);

    int exact = count_satisfying_assignments_exact(small_dnf, n_small);
    std::println("Exact satisfying assignments: {} / {}", exact, 1 << n_small);
    std::println("Fraction: {:.4f}\n", static_cast<double>(exact) / (1 << n_small));

    // Print all satisfying assignments
    std::println("Satisfying assignments:");
    ...
    for (int trials : {1000, 5000, 10000}) {
        ...
        std::println("  {:6d} trials:  naive = {:8.3f}  Karp-Luby = {:8.3f}",
    ...
    std::print("Random DNF (n={}, clauses={}, clause_size={}):\n",
               n_large, num_clauses, clause_size);
    // Print first 5 clauses as sample
    std::print("  Sample clauses: ");
    ...
    std::println("Convergence of Karp-Luby estimator:");
    std::println("  {:>10s}  {:>10s}  {:>10s}  {:>10s}",
    ...
    for (int trials : {100, 500, 1000, 5000, 10000}) {
        ...
        std::println("  {:10d}  {:10.2f}  {:10.2f}  {:10.3f}%",
    ...
    for (int sz : {2, 3, 5}) {
        ...
        std::println("  clause_size={}: exact={:3d}  K-L={:8.2f}  err={:.2f}",
