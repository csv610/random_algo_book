...
double volume_convex_body_approx(const Polytope& p, std::mt19937& rng,
    ...
    for (int phase = 0; phase < num_phases; ++phase) {
        ...
        if (phase < 5 || phase % 10 == 0) {
            println("    Phase {:2d}: radius = {:.6f}  fraction_inside = {:.4f}  "
                         "cumul_ratio = {:.6f}",
...
void demonstrate_volume() {
    ...
    println("  The L1 ball: |x_1| + ... + |x_d| <= 1  has volume 2^d/d!\n");
...

    {
        ...
        for (int samples : {100, 500, 1000, 5000, 10000}) {
            ...
            println("    {:6d} samples: est = {:.6f}  err = {:.6f}",
...

    {
        ...
        println("  Exact volume: {:.6f}", unit_cube_volume());
        ...
        for (int i = 0; i < 15; ++i) {
            ...
            println("    step {:2d}: ({:.4f}, {:.4f}, {:.4f})  inside={}",
    ...
    for (int d : {2, 3, 4}) {
        auto simp = standard_simplex(d);
        double exact = simplex_volume(d);
        println("  dim={} simplex (exact={:.6f}):", d, exact);
        ...
        for (int samples : {1000, 5000, 10000}) {
            ...
            println("    {:6d} samples (5 runs avg): est = {:.6f}  "
                         "rel_error = {:.2f}%",
                         samples, avg, rel_err);
        }
        println();
    ...
    // ---------------------------------------------------------------
    println("=== Part 7: Volume Comparison Table ===\n");
    println("  MC estimates (30k samples) vs exact analytical volumes.\n");
    println("  {:>4s}  {:>16s}  {:>16s}  {:>16s}",
    ...
    for (int d : {2, 3, 4, 5, 6}) {
        ...
        println("  {:4d}  {:7.4f} / {:<5.1f}  {:7.4f} / {:<7.4f}  {:7.4f} / {:<7.4f}",
                     d, est_c, 1.0,
