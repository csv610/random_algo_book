...
inline Matching blossom_matching(const Graph& g) {
    ...
    for (int v = 0; v < n; ++v) {
        if (match[v] == -1) {
            if (try_augment(v)) {
                ++aug_count;
                println("  Augmentation {} found", aug_count);
            }
        }
    }
    ...
    println("Is valid matching: {}",
        verify_matching(g, matching) ? "YES" : "NO");
    println("Maximum matching size: {}", matching.size());
    return matching;
}
...
inline void demonstrate_matchings() {
    println("+==============================================================+");
    println("|   Chapter 12: Matchings -- Random & Maximum                 |");
    println("+==============================================================+");
    ...
    // --- Bipartite graph ---
    {
        ...
        println("\n  Comparison:");
        println("    Random maximal:  {}", rm.size());
        println("    Maximum (aug):   {}", bm.size());
        println("    1/2-approx:      {}", am.size());
    }
    ...
    // --- Petersen graph ---
    {
        ...
        println("    Maximum:         {}", bm.size());
        println("    1/2-approx:      {}", am.size());
        println("    (Petersen matching number = 5)");
    }
