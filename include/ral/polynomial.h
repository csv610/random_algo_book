...
void demonstrate_polynomial() {
    ...
    println("--- Basic Operations (mod {}) ---", p);
    {
        Polynomial f({1, 2, 3});       // 3x^2 + 2x + 1
        Polynomial g({4, 5});          // 5x + 4

        println("  f(x) = {}", f.to_string());
        println("  g(x) = {}", g.to_string());

        Polynomial sum = f.add(g, p);
        println("  f + g = {}", sum.to_string());

        Polynomial diff = f.subtract(g, p);
        println("  f - g = {}", diff.to_string());

        Polynomial prod = f.multiply(g, p);
        println("  f * g = {}", prod.to_string());

        // Evaluate at x = 10 mod 97
        long long x = 10;
        println("  f({}) mod {} = {}", x, p, f.evaluate(x, p));
        println("  g({}) mod {} = {}", x, p, g.evaluate(x, p));
        println("  (f*g)({}) mod {} = {}", x, p, prod.evaluate(x, p));
    }
    ...
    println("\n--- Schwartz-Zippel: Zero Polynomial ---");
    {
        ...
        println("  Polynomial: 0 (all coefficients zero)");
        println("  Schwartz-Zippel says identically zero? {}", result ? "YES" : "NO");
    }
    ...
    println("\n--- Schwartz-Zippel: Non-Zero Polynomial ---");
    {
        ...
        Polynomial f({0, -1, 1});
        println("  Polynomial: {} = x(x-1)", f.to_string());
        println("  f(0) = {}, f(1) = {}, f(2) = {}",
    ...
    println("\n--- Schwartz-Zippel: Product of Many Roots ---");
    {
        ...
        println("  Polynomial degree: {}", f.degree());
        println("  Schwartz-Zippel says identically zero? {}",
    ...
    println("\n--- Polynomial Identity Testing ---");
    {
        ...
        println("  a(x) = {}", a.to_string());
        println("  b(x) = {}", b.to_string());
        println("  c(x) = {}", c.to_string());
        println("  a == b? {}", polynomial_identity_test(a, b, p, 30) ? "YES" : "NO");
        println("  a == c? {}", polynomial_identity_test(a, c, p, 30) ? "YES" : "NO");
    }
    ...
    println("\n--- Schwartz-Zippel Probability Bound ---");
    {
        ...
        long long p_val = 97;
        println("  Over Z_{} (p = {}):", p_val, p_val);
        println("    degree 1:  error <= {:.4f}", 1.0 / p_val);
        println("    degree 5:  error <= {:.4f}", 5.0 / p_val);
        println("    degree 10: error <= {:.4f}", 10.0 / p_val);
        println("    degree 50: error <= {:.4f}", 50.0 / p_val);
        println("  With 20 trials, degree 10: error <= {:.2e}", std::pow(10.0 / p_val, 20));
    }
}
