...
void demonstrate_min_cut() {
    ...
    println("Test 1: Small graph (known min-cut)");
    {
        ...
        println("  Single Karger run: {}", single);
        println("  Karger repeated {} times: {}", trials, karger_rep);
        println("  Karger-Stein (10 reps): {}", ks);

        double prob = 2.0 / (n * (n - 1));
        double fail_prob = std::pow(1.0 - prob, trials);
        println("  P(success one run) >= {:.6f}", prob);
    ...
    for (int n : {20, 50, 100}) {
        ...
        println("  n={} m={}:", n, m);
        println("    Exact (max-flow): {} ({:.1f}ms)", exact_val, exact_ms);
        println("    Karger (n^2/2 runs): {} ({:.1f}ms)", karger_val, karger_ms);
        println("    Karger-Stein: {} ({:.1f}ms)", ks_val, ks_ms);
    ...
    println("Test 3: Empirical success probability");
    {
        ...
        println("  n={}, exact min-cut={}", n, exact_val);
        println("  Theoretical P(success) per Karger run: {:.6f}", theoretical);
        println("  Empirical Karger success rate: {}/{} = {:.3f}",
                     success_karger, num_trials, static_cast<double>(success_karger) / num_trials);
        println("  Empirical Karger-Stein success rate: {}/{} = {:.3f}",
