...
void demonstrate_apsp() {
    ...
    println("Test 1: Small weighted graph (5 vertices)");
    {
        constexpr int n = 5;
        Matrix W(n, std::vector<double>(n, INF));
        for (int i : std::views::iota(0, n)) W[i][i] = 0;
        ...
        println("  Floyd-Warshall distances:");
        auto fw = floyd_warshall(W);
        for (int i : std::views::iota(0, n)) {
            print("    ");
            for (int j : std::views::iota(0, n))
                print("{:4}", fw[i][j] >= INF ? -1 : static_cast<int>(fw[i][j]));
            println("");
        }

        println("  Min-plus squaring distances:");
        auto mps = apsp_repeated_squaring(W);
        for (int i : std::views::iota(0, n)) {
            print("    ");
            for (int j : std::views::iota(0, n))
                print("{:4}", mps[i][j] >= INF ? -1 : static_cast<int>(mps[i][j]));
            println("");
        }
    ...
    println("Test 2: Timing comparison on random weighted graphs");
    for (int n : {50, 100, 200}) {
        ...
        println("  n={}: Floyd-Warshall={:.1f}ms, Min-plus-sq={:.1f}ms, match={}",
    ...
    println("\nTest 3: Unweighted graph (Seidel's boolean approach)");
    {
        ...
        println("  n={}: Seidel time={:.1f}ms, matches BFS: {}",
