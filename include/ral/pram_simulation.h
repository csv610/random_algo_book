...
public:
    ...
    std::vector<long long> list_ranking_parallel(const std::vector<int>& next_ptr) {
        ...
        for (int step = 0; step < max_steps; ++step) {
            ...
            println("Step {}: ranks = ", step + 1);
            print_list(next_cur, rank);
            if (!any_active) break;
        }

        println("Final list ranks:");
        for (int i = 0; i < n; ++i) {
            println("  node {} -> rank {}", i, rank[i]);
        }
        return rank;
    }
    ...
    std::vector<int> connected_components_parallel(
        ...
        println("Graph with {} vertices:", n);
        for (int i = 0; i < n; ++i) {
            if (!adj[i].empty())
                println("  {} -> [{}]", i, fmt_join(adj[i]));
        }
        ...
        unique_labels.erase(std::unique(unique_labels.begin(), unique_labels.end()),
                            unique_labels.end());
        println("Found {} connected component(s)", unique_labels.size());
        return label;
    }
...
inline void demonstrate_pram() {
    println("+==============================================================+");
    println("|   Chapter 12: PRAM Simulation -- Parallel Algorithms        |");
    println("+==============================================================+");
    ...
    // --- Parallel Prefix Sum ---
    {
        ...
        println("Parallel time:  {} us",
            std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count());
        println("Sequential time: {} us",
