...
int deterministic_lru(int cache_size, const std::vector<int>& requests, bool verbose = false) {
    ...
    for (int step = 0; auto page : requests) {
        ...
        if (cache.contains(page)) {
            cache.touch(page);
            if (verbose) {
                println("  Step {:2d}: page {} HIT   cache={}",
            ...
            cache.insert(page);
            if (verbose) {
                println("  Step {:2d}: page {} MISS  cache={}",
                             step, page, cache.snapshot());
            }
        }
    }
...
int marking_algorithm(int cache_size, const std::vector<int>& requests, bool verbose = false) {
    ...
    for (int step = 0; auto page : requests) {
        ...
        if (cache.contains(page)) {
            marked.insert(page);
            if (verbose) {
                println("  Step {:2d}: page {} HIT   cache={} marked={} round={}",
            ...
            marked.insert(page);
            if (verbose) {
                println("  Step {:2d}: page {} MISS  cache={} marked={} round={}",
                             step, page, cache.snapshot(), marked.size(), round);
            }
        }
    }
...
void demonstrate_paging() {
    ...
    println("--- Small Example: Cache size k=3 ---");
    println("Request sequence: {{1, 2, 3, 4, 1, 2, 5, 1, 2, 3, 4, 5}}\n");
    ...
    println("[Random Paging]");
    int rand_misses = random_paging(k, requests, true);
    println("  Total misses: {}\n", rand_misses);

    println("[Marking Algorithm]");
    int mark_misses = marking_algorithm(k, requests, true);
    println("  Total misses: {}\n", mark_misses);

    println("[OPT (Belady's Offline)]");
    int opt_misses = optimal_offline(k, requests, true);
    println("  Total misses: {}\n", opt_misses);

    // ---- Summary of small example ----
    println("--- Small Example Summary ---");
    println("  LRU:    {:2d} misses  ratio={:.2f}", lru_misses,
                 static_cast<double>(lru_misses) / opt_misses);
    println("  FIFO:   {:2d} misses  ratio={:.2f}", fifo_misses,
                 static_cast<double>(fifo_misses) / opt_misses);
    println("  Random: {:2d} misses  ratio={:.2f}", rand_misses,
                 static_cast<double>(rand_misses) / opt_misses);
    println("  Mark:   {:2d} misses  ratio={:.2f}", mark_misses,
                 static_cast<double>(mark_misses) / opt_misses);
    println("  OPT:    {:2d} misses", opt_misses);
    println("");

    // ---- Competitive ratio: adversarial sequence ----
    println("--- Competitive Ratio: Adversarial Sequences ---");
    println("For a k-page cache, an adversary can force any deterministic algorithm to incur k times the optimal cost.");
    println("LRU and FIFO are k-competitive but not (k-1)-competitive.\n");
    ...
    println("  Adversarial sequence (cycle of 4, length 20, k=3):");
    println("  LRU ratio:    {:.2f}", lru_ratio);
    println("  FIFO ratio:   {:.2f}", fifo_ratio);
    println("  Random ratio: {:.2f}", rnd_ratio);
    println("  Mark ratio:   {:.2f}", mark_ratio);
    println("");

    // ---- Average-case on random sequences ----
    println("--- Average Case: Random Sequences ---");
    ...
    for (int cache_k : {2, 3, 5}) {
        ...
        println("  k={}: seq_len={} num_pages={} ({} trials)", cache_k, seq_len, num_pages, num_trials);
        println("    LRU:    {:.1f} avg misses  (ratio {:.2f})", avg_lru, avg_lru / avg_opt);
        println("    FIFO:   {:.1f} avg misses  (ratio {:.2f})", avg_fifo, avg_fifo / avg_opt);
        println("    Random: {:.1f} avg misses  (ratio {:.2f})", avg_rand, avg_rand / avg_opt);
        println("    Mark:   {:.1f} avg misses  (ratio {:.2f})", avg_mark, avg_mark / avg_opt);
        println("    OPT:    {:.1f} avg misses", avg_opt);
        println("");
    }

    // ---- Scalability test ----
    println("--- Scalability: Sequence Length vs Misses ---");
    {
        int cache_k = 3;
        println("  Cache size k={}, {} pages, 50 trials per length\n",
                     cache_k, num_pages);

        println("  {:>10s}  {:>8s}  {:>8s}  {:>8s}  {:>8s}  {:>8s}",
        ...
        for (int len : {100, 500, 1000, 5000, 10000}) {
            ...
            println("  {:>10d}  {:>8.1f}  {:>8.1f}  {:>8.1f}  {:>8.1f}  {:>8.1f}",
    ...
    println("  LRU:    k-competitive (tight among deterministic algorithms)");
    println("  FIFO:   k-competitive");
    println("  Random: H_k-competitive (expected), ~ ln(k) for large k");
    println("  Marking: H_k-competitive (expected), randomized");
    println("  OPT:    optimal offline (cost = lower bound)\n");
}
