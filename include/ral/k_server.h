    ...
    std::map<std::pair<int, std::vector<int>>, long long>& memo)
{
    ...
    std::vector<int> sorted_pos = current_positions;
    std::sort(sorted_pos.begin(), sorted_pos.end());
    auto state_key = std::make_pair(req_idx, sorted_pos);
...
void demonstrate_k_server() {
    ...
    println("--- Line Metric (positions 0..{}) ---", n - 1);
    print("     ");
    for (int j = 0; j < n; ++j) print("{:>4d}", j);
    println("");
    for (int i = 0; i < n; ++i) {
        print("{:>3d}: ", i);
        for (int j = 0; j < n; ++j) print("{:>4d}", dist[i][j]);
        println("");
    }
    println("");
    ...
    print("  ");
    for (int r : requests) print("{} ", r);
    println("\n");

    // ---- Greedy (closest server) ----
    println("--- Greedy (Closest Server) ---");
    {
        ServerState state(k, {0, 1});
        println("  Initial: servers at {}", state.snapshot());
        ...
        for (int step = 0; auto req : requests) {
            ...
            if (hit) {
                println("  Step {:2d}: request {} -> HIT   servers={}",
                             step, req, state.snapshot());
            } else {
                auto& [from, to] = state.movements.back();
                println("  Step {:2d}: request {} -> MISS  move {}->{}  servers={}  cost={}",
    ...
    // ---- Randomized ----
    println("--- Randomized (Inverse-Distance Weighted) ---");
    {
        // Run multiple trials
        ...
        println("  {} trials, initial servers at {{0,1}}", trials);
        println("  Average cost: {:.1f}", static_cast<double>(total_cost) / trials);
        ...
        for (int step = 0; auto req : requests) {
            ...
            if (!hit) {
                auto& [from, to] = state.movements.back();
                println("    Step {:2d}: request {} -> move {}->{}  servers={}  cost={}",
                             step, req, from, to, state.snapshot(), dist[from][to]);
            } else {
                println("    Step {:2d}: request {} -> HIT   servers={}",
                             step, req, state.snapshot());
    ...
    println("--- Optimal Offline (DP, small instance) ---");
    {
        ...
        println("  Requests: {}", short_req);
        println("  Initial servers: {{0, 1}}");
        println("  Greedy cost: {}", greedy_state.total_cost);
        println("  Optimal cost: {}", opt_cost);
        println("  Competitive ratio (greedy/opt): {:.2f}",
                     static_cast<double>(greedy_state.total_cost) / opt_cost);
        println("");
    }

    // ---- Comparison across different k values ----
    println("--- Comparison: k=1,2,3 on line {{0..5}} ---");
    {
        std::vector<int> requests_long = {5, 0, 3, 1, 4, 2, 5, 0, 3, 1, 4, 2};
        println("  Requests: {}", requests_long);
        println("  {:>4s}  {:>8s}  {:>10s}  {:>8s}",
    ...
    // ---- Metric space: circle ----
    println("--- Alternative Metric: Circle (6 positions) ---");
    {
        ...
        println("  Circle of 6 positions, requests: {}", circ_req);

        // Greedy
        ServerState gs(2, {0, 1});
        for (int req : circ_req) gs = min_server_move(gs, req, cdist);
        println("  Greedy cost: {}", gs.total_cost);
        ...
        println("  Random avg:  {:.1f}", static_cast<double>(rt) / 20);
    ...
    println("  k-Server Conjecture: no randomized algorithm is k/(k-1)-competitive");
    println("  Deterministic lower bound: k (Manasse-McGeoch-Sleator)");
    println("  Randomized lower bound: k (Bartal)");
    println("  The k-server problem is a fundamental problem in online algorithm design.\n");
}
