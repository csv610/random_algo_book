...
inline IndependentSet mis_luby_parallel(const Graph& g) {
    ...
    while (true) {
        ...
        print("  Candidates: ");
        for (int i = 0; i < g.n; ++i)
            if (candidate[i]) print("{}({:.3f}) ", i, random_vals[i]);
        println();
...
inline Matching maximal_matching(const Graph& g) {
    ...
    println("  Processing {} edges:", all_edges.size());
...
inline std::vector<int> vertex_coloring(const Graph& g) {
    ...
    for (int v = 0; v < g.n; ++v) {
        ...
        println("  Vertex {} -> color {}", v, c);
    }

    println("Total colors used: {}", num_colors);
    ...
    println("Proper coloring: {}", proper ? "YES" : "NO");
    return color;
}
...
inline void demonstrate_mis() {
    println("+==============================================================+");
    println("|   Chapter 12: Maximal Independent Set & Matching           |");
    println("+==============================================================+");
