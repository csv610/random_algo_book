...
void demonstrate_adversary() {
    ...
    // ---- Oblivious Adversary ----
    println("--- Oblivious Adversary ---");
    println("The adversary fixes the entire sequence before the algorithm runs.\n");

    {
        int k = 3;
        auto worst_seq = ObliviousAdversary::worst_case_lru_fifo(k, 12);
        println("  Worst-case sequence for LRU/FIFO (k={}, length=12):", k);
        print("    ");
        for (int r : worst_seq) print("{} ", r);
        println("");

        auto result = adversarial_test(deterministic_lru, optimal_offline, k, worst_seq);
        println("  LRU cost:    {}  OPT: {}  ratio: {:.2f}",
        ...
        println("  FIFO cost:   {}  OPT: {}  ratio: {:.2f}",
        ...
        println("  Random cost: {}  OPT: {}  ratio: {:.2f} (one trial)",
                     result3.algorithm_cost, result3.optimal_cost, result3.competitive_ratio);
        println("");
    }

    // ---- Adaptive Adversary ----
    println("--- Adaptive Online Adversary ---");
    println("The adversary observes the algorithm's state at each step.\n");

    {
        ...
        print("    Adaptive sequence: ");
        for (int r : adseq) print("{} ", r);
        println("");
        ...
        println("  Random cost: {}  OPT: {}  ratio: {:.2f} (on LRU-adapted sequence)",
    ...
    println("--- Oblivious vs Adaptive Adversary ---\n");
    {
        int k = 3;
        ...
        println("  Oblivious adversary (fixed sequence, length 100):");
        println("    LRU avg ratio:    {:.2f}", oblivious_ratio_lru / trials);
        println("    Random avg ratio: {:.2f}", oblivious_ratio_rand / trials);
        ...
        println("    LRU ratio:    {:.2f}", adaptive_lru.competitive_ratio);
        println("    Random ratio: {:.2f}", adaptive_rand.competitive_ratio);
        println("");
    }

    // ---- Randomization Helps Against Oblivious Adversaries ----
    println("--- Why Randomization Helps ---\n");
    {
        int k = 4;
        println("  For k={}, deterministic algorithms can be forced to k-competitive.\n", k);
        ...
        println("  k={}, H_k = {:.4f} (~ ln(k) = {:.4f})", k, hk, std::log(k));
        println("  Deterministic LRU ratio:  {:.2f} (theory: {})", det_result.competitive_ratio, k);
        println("  Randomized avg ratio:     {:.2f} (theory: H_k = {:.2f})",
                     rand_avg_ratio / trials, hk);
        println("  Speedup from randomization: {:.2f}x",
                     det_result.competitive_ratio / (rand_avg_ratio / trials));
        println("");
    }

    // ---- Different Adversarial Strategies ----
    println("--- Different Adversarial Strategies ---\n");
    {
        int k = 3;
        ...
        println("  {:>14s}  {:>12s}  {:>12s}", "Strategy", "LRU ratio", "Random ratio");
        println("  {:>14s}  {:>12s}  {:>12s}", "--------", "---------", "-----------");
        for (auto& [name, lr, rr] : results)
            println("  {:>14s}  {:>12.2f}  {:>12.2f}", name, lr, rr);
        println("");
    }

    println("--- Summary ---");
    println("  1. Oblivious adversary: worst-case over all input sequences");
    println("  2. Adaptive adversary: worst-case over sequences that adapt to algorithm");
    println("  3. Randomization buys immunity against oblivious adversaries");
    println("  4. Against adaptive adversaries, randomization helps less");
    println("  5. The competitive ratio measures worst-case online performance\n");
}
