...
void demonstrate_number_theory() {
    ...
    println("--- Modular Exponentiation ---");
    {
        ...
        println("  2^10 mod 1000 = {}", mod_pow(base, exp, mod));

        base = 3; exp = 13; mod = 7;
        println("  3^13 mod 7    = {}", mod_pow(base, exp, mod));

        base = 7; exp = 256; mod = 13;
        println("  7^256 mod 13  = {}", mod_pow(base, exp, mod));
    }
    ...
    println("\n--- Extended GCD ---");
    {
        ...
        println("  35 * ({}) + 15 * ({}) = {} = gcd(35, 15)", x, y, g);

        auto [g2, x2, y2] = extended_gcd(30, 20);
        println("  30 * ({}) + 20 * ({}) = {} = gcd(30, 20)", x2, y2, g2);

        auto [g3, x3, y3] = extended_gcd(99, 78);
        println("  99 * ({}) + 78 * ({}) = {} = gcd(99, 78)", x3, y3, g3);
    }
    ...
    println("\n--- Trial Division Primality ---");
    {
        ...
        for (long long n : test_nums) {
            println("  is_prime({}) = {}", n, is_prime_trial(n) ? "true" : "false");
        }
    }
    ...
    println("\n--- Chinese Remainder Theorem ---");
    {
        ...
        println("  x == 2 (mod 3), x == 3 (mod 5), x == 2 (mod 7)");
        println("  Solution: x = {} (mod {})", x, 3 * 5 * 7);

        // Verify
        println("  Verify: {} mod 3 = {}, {} mod 5 = {}, {} mod 7 = {}",
                      x, x % 3, x, x % 5, x, x % 7);
    }
