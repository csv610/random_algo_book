...
We consider the tree as a balanced binary tree of AND/OR gates of depth~$2k$;
the NOR terminology belongs to a different basis and is not used here.
...
\begin{lemma}[Integer-weight reduction]
...
\label{sec:reservoir-sampling}
%% ============================================================

Many data sources are too large to fit in memory: network packet streams, log files, sensor readings, and database query results are examples of \emph{streams} that can only be read once and in order. We often need a uniform random sample of $k$ elements from such a stream without knowing its length $n$ in advance. \emph{Reservoir sampling} solves this problem elegantly.

\subsection{Algorithm~A: Uniform Reservoir Sampling}

The algorithm, attributed to Vitter (1985), maintains a reservoir of $k$ elements. When the $i$th element arrives (for $i \leq k$), it is placed directly into the reservoir. For $i > k$, it replaces an element in the reservoir with probability $k/i$.

\begin{algorithm}[htbp]
\caption{Reservoir Sampling (Algorithm~A)}
\label{alg:reservoir}
\begin{algorithmic}[1]
\State Initialize reservoir $R$ with the first $k$ elements of the stream
\For{$i = k+1, k+2, \ldots, n$}
  \State $j \gets \text{random integer in } \{1, 2, \ldots, i\}$
  \If{$j \leq k$}
    \State Replace $R[j]$ with the $i$th element
  \EndIf
\EndFor
\State \Return $R$
\end{algorithmic}
...
\begin{theorem}\label{thm:reservoir-uniform}
After processing $n$ elements, Algorithm~A produces a reservoir in which every subset of $k$ elements from the stream is equally likely to be the sample. Specifically, each element is included with probability exactly $k/n$.
\end{theorem}

\begin{proof}
We prove by induction on $i$ that after processing $i$ elements, every $k$-subset of the first $i$ elements is equally likely to be the reservoir. The base case $i = k$ is trivial (the reservoir is the first $k$ elements). For the inductive step, consider the $(i{+}1)$th element. With probability $k/(i{+}1)$, it replaces a uniformly random element of the reservoir. A direct calculation shows that each $k$-subset of the first $i{+}1$ elements has probability $\binom{i}{k}^{-1}$ of being the reservoir after the $(i{+}1)$th step. The inclusion probability of any fixed element is therefore $k/n$.
\end{proof}

The beauty of reservoir sampling is that it requires only $O(k)$ memory regardless of $n$, and each element is processed in $O(1)$ time. The total running time is $O(n)$.

\subsection{Algorithm~A-Res: Weighted Reservoir Sampling}

In many applications, elements have non-uniform importance. \emph{Weighted reservoir sampling} ensures that element $i$ is included with probability proportional to its weight $w_i$. Efraimidis and Spirakis (2006) proposed an elegant algorithm based on key values.

\begin{algorithm}[htbp]
\caption{Weighted Reservoir Sampling (Algorithm~A-Res)}
\label{alg:weighted-reservoir}
\begin{algorithmic}[1]
\State Initialize reservoir $R$ with the first $k$ elements, each with key $u_i^{1/w_i}$ where $u_i \sim \text{Uniform}(0,1)$
\For{$i = k+1, k+2, \ldots, n$}
  \State Compute key $k_i = u_i^{1/w_i}$ where $u_i \sim \text{Uniform}(0,1)$
  \State Let $j$ be the index of the element in $R$ with the smallest key
  \If{$k_i > k_j$}
    \State Replace $R[j]$ with the $i$th element (with key $k_i$)
  \EndIf
\EndFor
\State \Return $R$
\end{algorithmic}
...
\begin{theorem}\label{thm:weighted-reservoir}
After processing $n$ elements, the probability that element $i$ is in the reservoir is exactly $k \cdot w_i / \sum_{j=1}^n w_j$.
\end{theorem}

\begin{proof}
The key $k_i = u_i^{1/w_i}$ where $u_i \sim \text{Uniform}(0,1)$. Element $i$ is included iff its key is among the $k$ largest keys. The CDF of $k_i$ is $\Pr[k_i \leq t] = t^{w_i}$ for $t \in [0,1]$. The probability that $k_i$ is among the top $k$ keys out of $n$ is exactly $k \cdot w_i / \sum_j w_j$, which follows from the symmetry of the key distribution.
\end{proof}

\begin{cpplisting}
\textbf{C++ implementation:} The files \texttt{examples/example\_generic\_reservoir.cpp} and \texttt{examples/example\_weighted\_reservoir.cpp} demonstrate both uniform and weighted reservoir sampling on streams of strings and integers, respectively.
\end{cpplisting}

\begin{remark}
Reservoir sampling is a fundamental tool in data mining, machine learning (stochastic gradient descent on large datasets), and database query processing. The weighted variant is particularly useful for importance sampling and stratified sampling from streams.
\end{remark}
...
For a fixed set of unit demands, suppose each demand is assigned an
independently sampled path and the expected load on every edge is at most
$\mu$.  If the number of demands and path lengths are polynomially bounded
in $n$, then the maximum load is
$O(\mu+\log n/\log\log n)$ with high probability.  A competitive-ratio
statement requires a separate comparison with an offline routing optimum.
\end{theorem}
...
\begin{proof}
Knowing $p$ and $q$ gives $\lambda(n)=\operatorname{lcm}(p-1,q-1)$.
Since $\gcd(e,\lambda(n))=1$, the extended Euclidean algorithm computes
$d=e^{-1}\bmod\lambda(n)$, and RSA decryption is $c^d\bmod n$.
\iffalse
...
\begin{cpplisting}
\lstinputlisting[language=C++]{src/chapter7/main.cpp}
\end{cpplisting}
...
Conditioning on all coordinates except one nonzero coefficient shows that
the row equation holds for at most one of the two possible values of that
coordinate. Thus $\Pr[D\mathbf r=\mathbf 0]\leq 1/2$, and independence gives
a false-positive probability at most $2^{-t}$ after $t$ rounds.
\end{proof}
...
\begin{cpplisting}
\begin{lstlisting}[title={Freivalds' algorithm --- \texttt{src/chapter8/freivalds.h}}]
...
\begin{cpplisting}
\begin{lstlisting}[title={Polynomial identity testing --- \texttt{src/chapter8/polynomial.h}}]
...
Each of the $n - m + 1$ windows has a fingerprint collision with the pattern
fingerprint with probability at most $m/p$ (by the Schwartz--Zippel lemma
applied to the difference polynomial of degree $m-1$). Thus the expected
number of spurious matches is $O(nm/p)$. If a collision is verified by a
direct $O(m)$ comparison, the expected verification cost is $O(nm^2/p)$;
choosing $p=\Theta(m^2)$ makes the total expected time $O(n+m)$, while
$p=\Theta(m)$ only bounds the number of candidate matches, not the cost of
verifying them.
...
\begin{cpplisting}
\begin{lstlisting}[title={Rabin--Karp search --- \texttt{src/chapter8/rabin\_karp.h}}]
...
\label{sec:randomized-la}
%% ============================================================

Classical linear algebra operations---matrix multiplication, singular value decomposition, and low-rank approximation---are expensive for large matrices. Randomized algorithms exploit the structure of the data to produce approximate answers dramatically faster than deterministic methods, with controlled and quantifiable error.

\subsection{Randomized SVD}

The singular value decomposition (SVD) of an $m \times n$ matrix $A$ factors it as $A = U \Sigma V^T$, where $U$ and $V$ are orthogonal and $\Sigma$ is diagonal with singular values $\sigma_1 \geq \sigma_2 \geq \cdots \geq 0$. Computing the full SVD takes $O(m n \min(m,n))$ time, which is prohibitive for large matrices.

The randomized SVD algorithm computes a rank-$k$ approximation $A \approx U_k \Sigma_k V_k^T$ in $O(m n k)$ time, which is much faster when $k \ll \min(m,n)$.

\begin{algorithm}[htbp]
\caption{Randomized SVD}
\label{alg:rand-svd}
\begin{algorithmic}[1]
\Function{RandomizedSVD}{$A \in \mathbb{R}^{m \times n}$, target rank $k$, oversampling $p$}
  \State Draw random Gaussian matrix $\Omega \in \mathbb{R}^{n \times (k+p)}$
  \State $Y \gets A \Omega$ \Comment{$m \times (k{+}p)$ matrix}
  \State Compute QR factorization: $Y = Q R$
  \State $B \gets Q^T A$ \Comment{small matrix $(k{+}p) \times n$}
  \State Compute classical SVD: $B = \hat{U} \Sigma V^T$
...
\begin{theorem}\label{thm:rand-svd}
Let $A$ be an $m \times n$ matrix and let $k$ be the target rank. The randomized SVD algorithm (with oversampling $p \geq 5$) produces a rank-$k$ approximation satisfying
\[
\|A - U_k \Sigma_k V_k^T\|_2 \leq \sigma_{k+1}(A)
\]
with probability at least $1 - 3 n^{-p}$. The running time is $O(m n k)$.
\end{theorem}

The key insight is that the random projection $Y = A\Omega$ captures the range of $A$ with high probability. The QR factorization of $Y$ produces an approximate orthonormal basis $Q$ for this range, and the subsequent SVD of the small matrix $B = Q^T A$ identifies the dominant singular directions. The oversampling parameter $p$ provides a safety margin: larger $p$ increases the probability that $Q$ captures the relevant subspace.

\subsection{The Johnson--Lemma and Random Projections}

The \emph{Johnson--Lindenstrauss (JL) lemma} is one of the most surprising results in randomized algorithms: a small random linear map can approximately preserve all pairwise distances in a dataset.

\begin{theorem}[Johnson--Lindenstrauss]\label{thm:jl}
For any set $S$ of $n$ points in $\mathbb{R}^d$ and any $\varepsilon \in (0,1)$, there exists a linear map $f : \mathbb{R}^d \to \mathbb{R}^k$ with $k = O(\varepsilon^{-2} \log n)$ such that for all $u, v \in S$,
\[
(1 - \varepsilon) \|u - v\|^2 \leq \|f(u) - f(v)\|^2 \leq (1 + \varepsilon) \|u - v\|^2.
\]
The map is realized by a random Gaussian matrix $R \in \mathbb{R}^{k \times d}$ with entries drawn independently from $\mathcal{N}(0, 1/k)$.
\end{theorem}

\begin{proof}
For a fixed pair $(u,v)$, let $z = (u - v)/\|u - v\|$. The projected squared distance is $\|Rz\|^2 \cdot \|u-v\|^2$, where $Rz$ is a sum of $k$ independent random variables. Each component $(Rz)_i = \sum_j R_{ij} z_j$ is Gaussian with mean 0 and variance $1/k$. Thus $\|Rz\|^2 = \sum_{i=1}^k (Rz)_i^2$ is the sum of $k$ independent $\chi^2_1 / k$ random variables. By standard concentration bounds for $\chi^2$ random variables, $\Pr[|\|Rz\|^2 - 1| \geq \varepsilon] \leq 2 \exp(-k \varepsilon^2 / 4)$. Applying the union bound over all $\binom{n}{2}$ pairs and choosing $k \geq (4/\varepsilon^2) \ln \binom{n}{2}$ gives the result.
\end{proof}

The JL lemma has profound practical implications. A dataset of $n$ points in $\mathbb{R}^{1000}$ can be projected to $\mathbb{R}^{10}$ (a 99\% dimension reduction) while preserving all pairwise distances within 25\%, provided $n$ is not too large. This is the foundation of dimensionality reduction techniques used in machine learning, information retrieval, and data mining.

\subsection{SimHash}

\emph{SimHash} is a locality-sensitive hash for cosine similarity. It projects a vector onto $k$ random hyperplanes, producing a $k$-bit signature. Two vectors with cosine similarity $\cos \theta$ produce signatures that agree on each bit with probability $(1 - \theta/\pi)$.

\begin{algorithm}[htbp]
\caption{SimHash}
\label{alg:simhash}
\begin{algorithmic}[1]
\Function{SimHash}{$x \in \mathbb{R}^d$, random hyperplanes $h_1, \ldots, h_k$}
  \For{$i = 1$ to $k$}
    \State $s_i \gets \text{sign}(h_i \cdot x)$ \Comment{$+1$ or $-1$}
  \EndFor
...
The Hamming distance between two SimHash signatures is a good estimator of the angular distance between the original vectors. This makes SimHash ideal for near-duplicate detection, document similarity, and large-scale nearest neighbor search.

\begin{cpplisting}
\textbf{C++ implementation:} The files \texttt{examples/example\_randomized\_svd.cpp}, \texttt{examples/example\_jl\_projection.cpp}, \texttt{examples/example\_random\_projection.cpp}, and \texttt{examples/example\_simhash.cpp} demonstrate randomized SVD, JL dimension reduction, distance-preserving random projections, and SimHash, respectively.
\end{cpplisting}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Adversary qualification:} For an operation sequence fixed independently of the priorities, the expected cost is logarithmic. An adversary that observes or adapts to the hidden priorities can invalidate this guarantee, so the quantifiers in the model matter.
\end{enumerate}
...
\begin{cpplisting}
\textbf{C++ implementation:} See \texttt{src/chapter9/treap.h} for a complete implementation
...
\begin{cpplisting}
\textbf{C++ implementation:} See \texttt{src/chapter9/skip\_list.h} for a complete
...
\begin{cpplisting}
\textbf{C++ implementation:} See \texttt{src/chapter9/hash\_table.h} for a complete
...
\begin{cpplisting}
\textbf{C++ implementation:} See \texttt{src/chapter9/hash\_table.h} for an implementation
...
The nearest neighbor problem---given a query point, find the closest point in a dataset---is fundamental to information retrieval, machine learning, and computational geometry. Brute-force search takes $O(n d)$ time per query, which is prohibitive for large datasets. \emph{Locality-sensitive hashing} (LSH) provides sublinear-time approximate nearest neighbor search by hashing similar points to the same bucket with high probability.

\subsection{LSH Families}

\begin{definition}[Locality-sensitive family]
A family $\mathcal{H}$ of hash functions is \emph{$(r, cr, p_1, p_2)$-sensitive} for a distance metric $d$ if for all $x, y$:
\begin{itemize}[nosep]
  \item If $d(x,y) \leq r$, then $\Pr_{h \in \mathcal{H}}[h(x) = h(y)] \geq p_1$.
  \item If $d(x,y) > cr$, then $\Pr_{h \in \mathcal{H}}[h(x) = h(y)] \leq p_2$.
\end{itemize}
\end{definition}

By combining $k$ independent hash functions from the family and using $L$ independent hash tables, LSH achieves sublinear query time with provable recall guarantees.

\subsection{Hyperplane LSH for Cosine Similarity}

For cosine distance, the natural LSH family uses random hyperplanes. A random hyperplane through the origin partitions $\mathbb{R}^d$ into two half-spaces. Two vectors on the same side of all $k$ hyperplanes receive the same hash. The collision probability is $(1 - \theta/\pi)$ where $\theta$ is the angle between the vectors.

\begin{algorithm}[htbp]
\caption{Hyperplane LSH for Cosine Similarity}
\label{alg:hyperplane-lsh}
\begin{algorithmic}[1]
\Function{HyperplaneLSH}{point $x \in \mathbb{R}^d$, hyperplanes $h_1, \ldots, h_k$}
  \For{$i = 1$ to $k$}
    \State $b_i \gets \text{sign}(r_i \cdot x)$ where $r_i \sim \mathcal{N}(0, I_d)$
  \EndFor
  \State \Return bucket $(b_1, \ldots, b_k)$
\EndFunction
\end{algorithmic}
\end{algorithm}

\subsection{E2LSH for Euclidean Distance}

For $\ell_2$ distance, the \emph{E2LSH} family (Datar et al., 2004) uses $p$-stable distributions. Each hash function is
\[
h_{a,b}(x) = \left\lfloor \frac{a \cdot x + b}{w} \right\rfloor
\]
where $a$ is drawn from a standard normal (a 2-stable distribution) and $b \sim \text{Uniform}(0, w)$. The bucket width $w$ controls the trade-off between precision and recall: smaller $w$ gives finer granularity but more hash tables.

Points close in Euclidean distance hash to the same bucket with probability that decreases smoothly with distance. Specifically, for two points at distance $d$, the collision probability is
\[
\Pr[h(x) = h(y)] = \int_0^w \frac{1}{a} f\!\left(\frac{t}{a}\right) \left(1 - \frac{t}{w}\right) dt
\]
where $a = \|x - y\|$ and $f$ is the standard normal density. This function is monotonically decreasing in $d$, ensuring that closer points collide more often.

\subsection{LSH Index Construction}

An LSH index builds $L$ independent hash tables, each using $k$ hash functions from the family. Insertion hashes each point and stores it in the corresponding bucket of each table. A query hashes the query point and examines all candidates in the same bucket across all tables, ranking them by distance.

\begin{algorithm}[htbp]
\caption{LSH Index: Query}
\label{alg:lsh-query}
\begin{algorithmic}[1]
\Function{LSHQuery}{query $q$, tables $T_1, \ldots, T_L$, top-$k$}
  \State $C \gets \emptyset$ \Comment{candidate set}
  \For{$\ell = 1$ to $L$}
    \State $b \gets h_\ell(q)$ \Comment{hash using table $\ell$'s functions}
    \State $C \gets C \cup T_\ell[b]$ \Comment{add all points in bucket}
  \EndFor
  \State Sort $C$ by distance to $q$
  \State \Return top-$k$ closest points from $C$
\EndFunction
\end{algorithmic}
\end{algorithm}

The number of tables $L$ controls the recall: more tables means more candidates are examined, increasing the probability of finding the true nearest neighbor. The query time is $O(L \cdot k + |C|)$ where $|C|$ is the total candidate set size.

\begin{theorem}\label{thm:lsh-recall}
For $(r, cr, p_1, p_2)$-sensitive LSH with $k$ hash functions and $L$ tables, the recall@1 is at least $1 - (1 - p_1^k)^L$.
\end{theorem}

\begin{cpplisting}
\textbf{C++ implementation:} The files \texttt{examples/example\_hyperplane\_lsh.cpp}, \texttt{examples/example\_e2lsh.cpp}, and \texttt{examples/example\_lsh\_index.cpp} demonstrate hyperplane LSH for cosine similarity, E2LSH for Euclidean distance, and a complete LSH index for approximate nearest neighbor search.
\end{cpplisting}
...
Unless stated otherwise, geometric algorithms in this chapter use the real-RAM
model with exact orientation and incircle predicates. General position means
that no three input points are collinear and no four are cocircular; for
degenerate inputs, a symbolic perturbation or deterministic tie-breaking rule
must be specified before uniqueness claims are made.
...
Given a set $P$ of $n$ points in the plane, the \emph{minimum enclosing circle} (MEC) problem asks for the smallest circle that contains all points. This problem arises in facility location, collision detection, and clustering.

The minimum enclosing circle is unique and is determined by at most three points on its boundary (called \emph{support points}). If two points define the circle, they are diametrically opposite; if three points define it, they form an acute triangle and the circle is its circumcircle.

\subsection{Welzl's Algorithm}

Welzl (1991) gave a beautifully simple randomized incremental algorithm. The idea is to process points in random order, maintaining the current MEC. When a new point falls outside the current MEC, we must rebuild the circle with the new point on the boundary, requiring us to check all previously processed points.

\begin{algorithm}[htbp]
\caption{Welzl's Minimum Enclosing Circle}
\label{alg:welzl}
\begin{algorithmic}[1]
\Function{WelzlMEC}{$P$} \Comment{points in random order}
  \State Shuffle $P$ randomly
  \State $C \gets$ circle through $P[0], P[1]$ \Comment{diameter circle}
  \For{$i = 2$ to $|P|-1$}
    \If{$P[i]$ is outside $C$}
      \State $C \gets$ MEC of $\{P[0], \ldots, P[i]\}$ with $P[i]$ on boundary
    \EndIf
  \EndFor
  \State \Return $C$
\EndFunction
...
The key subroutine determines the MEC given that one boundary point is fixed. It processes the remaining points in random order, maintaining the current circle with the fixed point on the boundary. If a point falls outside, the circle is rebuilt with the new point also on the boundary.

\begin{theorem}\label{thm:welzl-mec}
Welzl's algorithm computes the minimum enclosing circle of $n$ points in the plane in $O(n)$ expected time.
\end{theorem}

\begin{proof}
The analysis uses backward analysis. Consider the moment when the $i$th point is processed and causes a rebuild. The new circle must have this point on its boundary. Among the $i$ processed points, the probability that the specific point that triggered the rebuild was the last to arrive is exactly $1/i$. Therefore, the expected number of rebuilds triggered by the $i$th point is at most $O(1/i)$, and the total expected number of rebuilds is $\sum_{i=1}^n O(1/i) = O(\log n)$. Each rebuild takes $O(i)$ time, giving a total expected time of $\sum_{i=1}^n O(1/i) \cdot O(i) = O(n)$.
\end{proof}

The algorithm's simplicity and linear expected time make it the method of choice for MEC computation. Its randomized nature eliminates the worst-case $O(n^2)$ behavior of deterministic incremental algorithms.

\begin{cpplisting}
\textbf{C++ implementation:} The file \texttt{examples/example\_welzl\_mec.cpp} implements Welzl's algorithm with the iterative variant and verifies correctness against an $O(n^4)$ brute-force approach.
\end{cpplisting}
...
\]
always, where $Z(x)$ is the exact answer. A PTAS is a \emph{fully polynomial PTAS} (FPTAS) if its running time is polynomial in both the input size and $1/\varepsilon$.
\end{definition}
...
A \emph{fully polynomial almost-uniform sampler} (FPAUS) for a succinctly
represented finite set $\Omega$ is a randomized algorithm that, given an error
parameter $\varepsilon > 0$, outputs a sample from a distribution $\hat{D}$
on $\Omega$ satisfying
\[
\bigl\| \hat{D} - \text{Unif}(\Omega) \bigr\|_{\mathrm{TV}} \;\leq\; \varepsilon,
\]
where $\|\cdot\|_{\mathrm{TV}}$ denotes total variation distance, and whose
running time is polynomial in the input length and $1/\varepsilon$.
\end{definition}
...
Counting and sampling are related, but neither task implies the other for an
arbitrary implicitly represented family. The standard equivalence requires
self-reducibility and efficient access to conditional subinstances.

\begin{proposition}[Conditional counting--sampling connection]
\label{prop:count-sampling}
Let $\Omega(x)$ be a self-reducible solution family: fixing a prefix of a
solution produces a smaller instance, membership of a completed solution is
efficiently testable, and the number of branches at every decomposition step
is polynomially bounded. Under these conditions, an FPRAS for the associated
conditional counts yields an FPAUS by choosing each next branch with
probability proportional to its estimated conditional count. Conversely, an
FPAUS together with a polynomial-length sequence of overlapping, efficiently
samplable conditional distributions yields an FPRAS by estimating successive
ratios. Without these conditions, no general equivalence is claimed.
\end{proposition}

\begin{proof}[Proof sketch]
For the FPRAS-to-sampler direction, at a partial assignment $u$ let
$Z(u0)$ and $Z(u1)$ be the two conditional counts. Approximate both counts
to sufficiently small relative error and choose the next bit using the
estimated ratio. A union bound over the polynomially many levels controls the
accumulated total-variation error. The reverse direction requires an assumed
overlapping sequence of conditional distributions, so that each ratio is a
bounded-variance importance-sampling estimate; a coupon-collector argument
alone is exponential in $|\Omega|$.
\end{proof}
...
For a finite, lazy, reversible Markov chain with stationary distribution
$\pi$, conductance $\Phi>0$, and minimum stationary mass
$\pi_{\min}=\min_{\omega}\pi(\omega)$,
\[
\tau_{\mathrm{mix}}(\varepsilon)
=O\!\left(\frac{\log(1/(\varepsilon\pi_{\min}))}{\Phi^2}\right).
\]
\end{theorem}
...
Let $A$ be an $n \times n$ non-negative integer matrix with entries at most
$c$. In time polynomial in $n$ and $c$, one can construct a bipartite
multigraph whose number of perfect matchings is exactly $\operatorname{perm}(A)$.
\end{lemma}

\begin{proof}
Use $a_{ij}$ parallel copies of the edge $(u_i,v_j)$. A perfect matching
chooses a permutation of the right vertices and, for every selected pair,
one of the $a_{ij}$ parallel edge copies. Hence each permutation contributes
$\prod_i a_{i,\sigma(i)}$, and summing over permutations gives exactly
$\operatorname{perm}(A)$. Selector gadgets can replace parallel copies when a
simple graph is required, at polynomial size cost when $c$ is polynomially
bounded.
\end{proof}

\subsection{The Jerrum--Sinclair--Vazirani Markov chain}

The breakthrough result of Jerrum, Sinclair, and Vigoda constructs an FPRAS
for the permanent of every non-negative matrix. The full proof is a
research-level result; this chapter states the interface precisely and
isolates the Markov-chain ideas used by the theorem.
...
The key insight is to use a sequence of carefully weighted matching chains,
with state spaces containing matchings of adjacent cardinalities. The weights
are chosen so that the stationary mass of each level is known; ratios of the
level masses then reveal the desired count. A naive uniform chain on
$M_n\cup M_{n-1}$ is not enough.
...
The displayed moves are only a schematic adjacency relation. Their transition
probabilities are not symmetric when the numbers of neighboring matchings
differ, so uniformity on $M_n\cup M_{n-1}$ does not follow. The
Jerrum--Sinclair--Vigoda construction uses laziness, normalized transitions,
and explicit weights chosen to satisfy detailed balance.
\end{remark}

\begin{theorem}[Permanent FPRAS interface]
\label{thm:jsv-mixing}
For a non-negative matrix $A$, the weighted matching-chain construction of
Jerrum--Sinclair--Vigoda supplies polynomial-time approximate sampling and
ratio estimation, and therefore an FPRAS for $\operatorname{perm}(A)$.
\end{theorem}

The proof proceeds via the conductance method (Theorem~\ref{thm:conductance})
for the weighted, lazy chains in the annealing sequence.
...
The technical analysis uses canonical paths and alternating-path structure for
the weighted chains. It is not obtained by treating the schematic moves as a
uniform random walk.

The JSV theorem supplies a polynomial conductance lower bound for each
weighted chain. Its canonical-path proof is a research-level ingredient and
is cited rather than replaced by a state-count argument.

The conductance theorem gives polynomial burn-in because the relevant
stationary-mass and conductance bounds are controlled by the encoded input,
not by enumerating the exponentially large state space.
...
An annealing sequence with polynomial overlap estimates successive ratios.
Allocating relative error and failure probability over its polynomially many
levels gives the FPRAS. The complete canonical-path construction is cited as
the JSV result rather than reproduced here.
\end{proof}

\begin{remark}
The achievement is that the weighted chains mix in time polynomial in the
encoded matrix size even though their state spaces may be exponentially large.
\end{remark}

\subsection{Estimating the permanent from the chain}

For an exactly uniform chain on $M_n\cup M_{n-1}$, the fraction of states in
$M_n$ would be $m_n/(m_n+m_{n-1})$. In the actual weighted chain, the known
stationary weights and the burn-in and dependence corrections must be used;
the displayed ratio is intuition, not an implementation recipe.
...
%% ============================================================
\section{Streaming Algorithms: Flajolet--Martin and Misra--Greis}
\label{sec:streaming}
%% ============================================================

In many applications, data arrives as a stream and we can only afford to use memory proportional to a small function of the input parameters, not the stream length. Two fundamental streaming problems are \emph{distinct element counting} and \emph{heavy hitter detection}.

\subsection{Flajolet--Martin Distinct Element Counting}

The \emph{distinct elements problem} asks: given a stream of elements from a universe $U$, estimate the number of distinct elements seen so far. The trivial solution requires $\Omega(n)$ space to store all seen elements. The Flajolet--Martin algorithm (1984) uses only $O(\log m)$ bits of space, where $m = |U|$.

The key idea is to hash each element to a binary string of length $\log m$ and examine the position of the least significant 1-bit. If we have seen $d$ distinct elements, the expected position of the leftmost 1-bit among all hashes is approximately $\log_2 d$.

\begin{algorithm}[htbp]
\caption{Flajolet--Martin Distinct Element Counting}
\label{alg:flajolet-martin}
\begin{algorithmic}[1]
\State Initialize bitmap $B[0..\log m]$ to all zeros
\For{each element $x$ in the stream}
  \State $h \gets \text{hash}(x)$ \Comment{hash to $\{0,1\}^{64}$}
  \State $p \gets \text{position of least significant 1-bit in } h$
  \State $B[p] \gets 1$
\EndFor
\State $\phi \gets \text{index of least zero bit in } B$
\State \Return $2^\phi / \psi$ \Comment{$\psi \approx 0.7735$ is a correction factor}
\end{algorithmic}
\end{algorithm}

\begin{theorem}\label{thm:fm}
The Flajolet--Martin estimator has expected relative error $O(1/\sqrt{p})$ where $p$ is the number of independent hash functions (registers) used. Using group-wise averaging with $p = 64$ registers gives relative error approximately $12.5\%$.
\end{theorem}

The algorithm uses $O(p \log m)$ bits of space and processes each element in $O(1)$ time, making it suitable for high-speed network monitoring and database query processing.

\subsection{Misra--Greis Heavy Hitter Detection}

The \emph{heavy hitter problem} asks: given a stream of items, find all items that appear more than $n/k$ times, using only $O(k)$ space. The Misra--Greis algorithm (1982) maintains $k$ counters and achieves this with strong guarantees.

\begin{algorithm}[htbp]
\caption{Misra--Greis Heavy Hitter Detection}
\label{alg:misra-gries}
\begin{algorithmic}[1]
\Function{MisraGreis}{stream, parameter $k$}
  \State Initialize $k$ counters, all zero
  \For{each element $x$ in the stream}
    \If{$x$ has a counter}
      \State Increment its counter
    \ElsIf{fewer than $k$ counters are active}
      \State Create a new counter for $x$ with value 1
    \Else
      \State Decrement all counters by 1
      \State Remove any counters that reach zero
    \EndIf
  \EndFor
  \State \Return all counters with their estimated frequencies
\EndFunction
\end{algorithmic}
\end{algorithm}

\begin{theorem}\label{thm:misra-gries}
The Misra--Greis algorithm finds all items with frequency $> n/k$ (no false negatives), and no item with frequency $< n/k - n/k^2$ is reported. The estimated counts are within $n/k$ of the true counts.
\end{theorem}

The algorithm processes each element in $O(1)$ time and uses $O(k)$ space, making it suitable for real-time network traffic analysis, log processing, and database query optimization.

\begin{cpplisting}
\textbf{C++ implementation:} The files \texttt{examples/example\_flajolet\_martin.cpp} and \texttt{examples/example\_misra\_gries.cpp} demonstrate distinct element counting on simulated network traffic and heavy hitter detection on packet streams.
\end{cpplisting}
...
\lstinputlisting[
  ...
  label={lst:dnf-counting}]{src/chapter12/dnf_counting.h}

\lstinputlisting[
  language=C++,
  caption={Volume estimation via hit-and-run and DFK annealing.},
  label={lst:volume}]{src/chapter12/volume.h}
...
\State Each vertex $v \in V'$ picks an independent priority $r(v)$ uniformly from $\{1,\ldots,n^3\}$
\State \textit{// Stage 2: Select local maxima}
\State Vertex $v$ is a \emph{candidate} if $r(v)>r(u)$ for every neighbor $u\in N_{V'}(v)$
...
The obsolete degree-dependent marking analysis is not used by the priority
version above. In particular, the probability $1/d_v$ belongs to a different
variant of Luby's algorithm and should not be mixed with the local-maximum
priority rule. The standard charging lemma for independent random priorities
is the analysis used here.
...
\label{thm:isolating}
Let $G = (U, V, E)$ be a bipartite graph with $|U| = |V| = n$, and suppose $G$ has at least one perfect matching. Assign each edge $e \in E$ an independent random weight $w(e)$ drawn uniformly from $\{1, 2, \ldots, 2n\}$. Then with probability at least $1/2$, there is a unique perfect matching of minimum weight.
\end{theorem}
...
Now let $M^*$ be the minimum-weight perfect matching under the random
weights. By a union bound over all pairs:
\[
\Pr[\text{minimum-weight perfect matching is not unique}] \leq \sum_{M_2 \neq M^*} \Pr[w(M^*) = w(M_2)] \leq \frac{|\mathcal{M}| - 1}{2n}.
\]

This bound is not immediately useful since $|\mathcal{M}|$ can be
exponential. Instead, we use a more refined argument. Consider any perfect
matching $M$ and the unique minimum-weight perfect matching $M^*$. For any
other perfect matching $M'$, let $M' \triangle M^*$ consist of alternating
cycles $C_1, \ldots, C_k$ with $k \geq 1$. Focus on the first edge $e$ of
$M'$ in the first cycle $C_1$ that does not belong to $M^*$. Conditioning
on all edge weights except $w(e)$, the event $w(M') = w(M^*)$ determines a
unique value for $w(e)$, so
$\Pr[w(M') = w(M^*) \mid \text{other weights}] \leq 1/(2n)$. Taking the
union bound over all $M' \neq M^*$:
\[
...
\fi
The standard Isolating Lemma states that these independent weights isolate
the minimum perfect matching with probability at least $1/2$.  Its proof
uses a witness/critical-interval argument; a union bound over all perfect
matchings is invalid because their number may be exponential.
\end{proof}
...
\State Assign independent random values in a sufficiently large field to the nonzero entries of the Tutte matrix
\State Evaluate the determinant polynomial; a nonzero value certifies a perfect matching
\State Repeat with fresh values to reduce the one-sided error
\State Recover a matching by self-reduction: for each edge, test whether deleting it preserves a perfect matching
\State Return the retained edges
\EndProcedure
\end{algorithmic}
\end{algorithm}

For a bipartite graph, the symbolic biadjacency determinant is a polynomial of
degree $n$. Distinct perfect matchings give distinct monomials, so this
polynomial is identically zero exactly when no perfect matching exists.

The determinant is used only as a randomized existence oracle. To recover an
actual matching, use self-reduction: for each candidate edge, test whether the
remaining graph still has a perfect matching, retain the edge when the answer
is yes, and delete its two incident vertices. At most $n$ stages are needed,
and the oracle error is amplified so that the union of all false-positive
probabilities is at most the requested failure probability.
...
For a bipartite graph $G$ with adjacency matrix $A$, replace every nonzero
entry by an independent variable. The resulting determinant polynomial is
identically zero if and only if $G$ has no perfect matching. If a perfect
matching exists, it is a nonzero polynomial of total degree $n$.
\end{theorem}

\begin{proof}
Distinct perfect matchings give distinct determinant monomials, so they cannot
cancel identically. If no perfect matching exists, every determinant term
contains a zero entry and the polynomial is identically zero.
\end{proof}
...
\begin{proof}[Proof sketch]
The determinant oracle is evaluated over a field large enough for
Schwartz--Zippel to give one-sided false-zero probability at most
$n/|\mathbb F|$. The standard parallel isolation and self-reduction
construction combines this oracle with polylogarithmic-depth matching
recovery. A sequential deletion of edges would only give polynomial depth,
and an adjugate does not identify a minimum-weight matching; neither is used.
\end{proof}
...
The exact-matching result is a standard randomized parallel theorem: encode
the red-edge count in a polynomial identity, use random evaluations to avoid
cancellation, and apply the parallel isolation/self-reduction machinery. The
polynomial has degree at most $n$, so Schwartz--Zippel controls each false
identity test. This is the Karp--Vazirani--Vazirani construction cited below;
we do not assume that the permanent itself is in $\mathbf{RNC}$.
\end{proof}
...
Randomized Byzantine agreement is a family of model-dependent protocols.
Known results can circumvent deterministic fixed-round limitations under
specified synchrony, authentication, and common-coin assumptions, but no
round bound follows from random majority voting alone.
...
\lstinputlisting[
  ...
  label={lst:pram}]{src/chapter13/pram_simulation.h}

\lstinputlisting[
  language=C++,
  caption={Luby's maximal independent set algorithm and vertex coloring.},
  label={lst:mis}]{src/chapter13/mis.h}

\lstinputlisting[
  language=C++,
  caption={Random and maximum matching algorithms.},
  label={lst:matchings}]{src/chapter13/matchings.h}
...
\lstinputlisting[
  ...
  label={lst:paging}]{src/chapter14/paging.h}

\lstinputlisting[
  language=C++,
  caption={$k$-server: greedy, randomized, and optimal offline algorithms.},
  label={lst:kserver}]{src/chapter14/k_server.h}

\lstinputlisting[
  language=C++,
  caption={Adversary simulation: oblivious and adaptive models.},
  label={lst:adversary}]{src/chapter14/adversary.h}
...
One may instead choose $e$ coprime to $\phi(n)$ and compute the inverse
modulo $\phi(n)$. Note that $\gcd(e,\lambda(n))=1$ does not in general imply
$\gcd(e,\phi(n))=1$; the two key-generation conventions should not be
confused. The correctness proof above uses the inverse modulo $\lambda(n)$.
...
\begin{theorem}[Factoring breaks textbook RSA]
\label{thm:rsa-factoring}
Given the factorization $n=pq$, one can solve RSA inversion in polynomial
time by computing $d=e^{-1}\bmod\lambda(n)$ and returning $c^d\bmod n$.
\end{theorem}
...
\begin{remark}
Factoring is sufficient to break textbook RSA, but the converse reduction is
not known in this generality. Thus RSA security is based on a computational
assumption related to, but not proved equivalent to, factoring.
\end{remark}
...
Fix an arbitrary choice of $r_1, \ldots, r_{n-1} \in S$. If all
$g_i(r_1, \ldots, r_{n-1}) = 0$, then $f(r_1, \ldots, r_{n-1}, r_n) = 0$
for all $r_n$. If at least one $g_i(r_1, \ldots, r_{n-1}) \neq 0$, then
$f(r_1, \ldots, r_{n-1}, x_n)$ is a nonzero polynomial in $x_n$ of degree
at most $k \leq d$, and has at most $d$ roots in $S$.

By the inductive hypothesis,
$\Pr\!\left[\bigwedge_{i} g_i(r_1, \ldots, r_{n-1}) = 0\right] \leq \frac{d - j}{|S|} \leq \frac{d}{|S|}$
(since some $g_i$ with $i \leq j$ is nonzero of degree at most $d - j$).
In the complementary event, $\Pr[f(r_1, \ldots, r_n) = 0 \mid r_1, \ldots,
r_{n-1}] \leq d/|S|$. By the law of total probability:
\[
...
If $n$ is an odd prime, the Miller--Rabin test always returns ``prime.'' If
$n$ is an odd composite, the standard strong-liar theorem bounds the passing
bases by one quarter of the reduced residue system; bases with nontrivial gcd
are witnesses. With uniform sampling from $\{2,\ldots,n-2\}$ and the small
cases handled separately, one round has false-positive probability at most
$1/4$ up to the endpoint adjustment, and $k$ independent rounds have error at
most $4^{-k}$.
\end{theorem}
...
\]
The standard Euler-liar lemma states that $L$ has size at most
$\varphi(n)/2$. One proves this by applying the Chinese remainder theorem to
the prime-power factors of $n$: if every unit were an Euler liar, the power
map would agree with the Jacobi character on the whole unit group, which
would force the prime-modulus Euler criterion simultaneously at all factors
and contradict compositeness. Therefore at least half of the units are
witnesses. The full factor-by-factor argument is the group-theoretic lemma
behind Solovay--Strassen; the invalid shortcut that $L$ is automatically a
subgroup is not needed.
\end{proof}
...
Polynomial multiplication is a fundamental operation in coding theory, signal processing, and number theory. Given two degree-$n$ polynomials, naive multiplication takes $O(n^2)$ time. Two randomized (and deterministic) approaches achieve faster multiplication: the Karatsuba algorithm and the Fast Fourier Transform.

\subsection{Karatsuba Multiplication}

The Karatsuba algorithm (1960) is a divide-and-conquer approach that reduces polynomial multiplication to three half-size multiplications instead of four, achieving $O(n^{\log_2 3}) \approx O(n^{1.585})$ time.

Given polynomials $A(x) = A_1(x) \cdot B^{n/2} + A_0(x)$ and $B(x) = B_1(x) \cdot B^{n/2} + B_0(x)$, we compute:
\[
A(x) \cdot B(x) = z_2 \cdot B^n + z_1 \cdot B^{n/2} + z_0
\]
where $z_2 = A_1 \cdot B_1$, $z_0 = A_0 \cdot B_0$, and $z_1 = (A_1 + A_0)(B_1 + B_0) - z_2 - z_0$. The three multiplications $z_0$, $z_1$, $z_2$ are computed recursively, giving the recurrence $T(n) = 3T(n/2) + O(n)$, which solves to $O(n^{\log_2 3})$.

\begin{cpplisting}
\textbf{C++ implementation:} The file \texttt{examples/example\_karatsuba.cpp} implements Karatsuba multiplication for polynomials over $\mathbb{Z}_p$ and verifies correctness against naive multiplication.
\end{cpplisting}

\subsection{The Fast Fourier Transform}

The Fast Fourier Transform (FFT) computes the discrete Fourier transform of a length-$n$ vector in $O(n \log n)$ time, compared to $O(n^2)$ for the naive algorithm. The FFT enables polynomial multiplication in $O(n \log n)$ time by converting coefficient representations to point-value representations, multiplying pointwise, and converting back.

The key insight is the \emph{divide-and-conquer} decomposition of the DFT. Let $\omega_n = e^{2\pi i/n}$ be a primitive $n$th root of unity. For an even-length DFT, we split the input into even and odd indexed elements:
\[
\hat{x}_k = \sum_{j=0}^{n-1} x_j \omega_n^{jk} = \sum_{j=0}^{n/2-1} x_{2j} \omega_{n/2}^{jk} + \omega_n^k \sum_{j=0}^{n/2-1} x_{2j+1} \omega_{n/2}^{jk}.
\]
This reduces one DFT of size $n$ to two DFTs of size $n/2$, giving the recurrence $T(n) = 2T(n/2) + O(n) = O(n \log n)$.

For polynomial multiplication, we evaluate both polynomials at $2n$ roots of unity using the FFT, multiply the values pointwise, and interpolate back using the inverse FFT. The total time is $O(n \log n)$.

\begin{theorem}\label{thm:fft-mul}
Two polynomials of degree $n$ can be multiplied in $O(n \log n)$ time using the Fast Fourier Transform.
\end{theorem}

The FFT is one of the most important algorithms in computer science, with applications in signal processing, image compression (JPEG), error-correcting codes, and large integer arithmetic. The \emph{number-theoretic transform} (NTT) is a variant that works over finite fields $\mathbb{Z}_p$, avoiding floating-point arithmetic.

\begin{cpplisting}
\textbf{C++ implementation:} The file \texttt{examples/example\_fft.cpp} demonstrates FFT-based polynomial multiplication and its inverse.
\end{cpplisting}
...
\label{ch:simulated-annealing}
%==========================================================================

Many optimization problems in computer science require finding the minimum or maximum of a cost function over a large combinatorial space. For NP-hard problems such as the traveling salesman problem (TSP) and the knapsack problem, no polynomial-time algorithm is known that finds the optimal solution. \emph{Simulated annealing} (SA) is a randomized metaheuristic that explores the solution space by accepting worse solutions early (to escape local optima) and gradually narrowing to improvements as the system ``cools.''

The name comes from an analogy with metallurgical annealing: a heated metal is slowly cooled, allowing atoms to settle into a low-energy crystalline structure. In SA, the ``temperature'' parameter controls the probability of accepting worse solutions, and this temperature is decreased over time according to a cooling schedule.

%% ============================================================
\section{The Simulated Annealing Framework}
\label{sec:sa-framework}
%% ============================================================

Let $S$ be a finite set of candidate solutions, and let $f : S \to \mathbb{R}$ be a cost function to minimize. SA maintains a current solution $s \in S$ and iteratively improves it.

\begin{algorithm}[htbp]
\caption{Simulated Annealing}
\label{alg:simulated-annealing}
\begin{algorithmic}[1]
\Function{SimulatedAnnealing}{$s_0$, $f$, $\text{neighbor}$, $T_0$, $\alpha$, $T_{\min}$, $\text{iter}$}
  \State $s \gets s_0$, $s^* \gets s_0$ \Comment{current and best solutions}
  \State $T \gets T_0$ \Comment{initial temperature}
  \While{$T > T_{\min}$}
    \For{$j = 1$ to $\text{iter}$}
      \State $s' \gets \text{neighbor}(s)$
      \State $\Delta f \gets f(s') - f(s)$
      \If{$\Delta f < 0$}
        \State $s \gets s'$ \Comment{always accept improvements}
      \ElsIf{$\text{random}(0,1) < e^{-\Delta f / T}$}
        \State $s \gets s'$ \Comment{accept worse with probability $e^{-\Delta f/T}$}
      \EndIf
      \If{$f(s) < f(s^*)$}
        \State $s^* \gets s$ \Comment{update best}
      \EndIf
    \EndFor
    \State $T \gets \alpha \cdot T$ \Comment{geometric cooling}
  \EndWhile
  \State \Return $s^*$
\EndFunction
\end{algorithmic}
...
\begin{itemize}[nosep]
  \item \textbf{Initial temperature $T_0$:} Should be large enough that most proposed moves are accepted, allowing broad exploration.
  \item \textbf{Cooling rate $\alpha$:} Typically $\alpha \in [0.99, 0.9999]$. Slower cooling (larger $\alpha$) gives better solutions but requires more time.
  \item \textbf{Neighbor function:} Defines the local structure of the search space. The quality of the neighbor function often determines SA's practical performance.
\end{itemize}

\begin{theorem}[Convergence of SA]\label{thm:sa-convergence}
If the cooling schedule satisfies $T(t) \geq c / \log(1 + t)$ for a suitable constant $c$ depending on the problem, then SA converges to the global optimum with probability 1 as $t \to \infty$.
\end{theorem}

\begin{proof}
At temperature $T$, the stationary distribution of the SA Markov chain assigns to each state $s$ a probability proportional to $e^{-f(s)/T}$. As $T \to 0$, this distribution concentrates on the global minimum: the probability of being in a state with cost $f(s) = f^* + \delta$ is at most $e^{-\delta/T}$, which vanishes as $T \to 0$. The logarithmic cooling schedule ensures that the chain mixes at each temperature before cooling further, guaranteeing convergence.
\end{proof}

In practice, the theoretical cooling schedule $O(1/\log t)$ is too slow. The geometric schedule $T_{k+1} = \alpha T_k$ with $\alpha = 0.99$ to $0.9999$ works well for most problems, providing a good balance between solution quality and running time.

%% ============================================================
\section{Application: The Traveling Salesman Problem}
\label{sec:sa-tsp}
%% ============================================================

The TSP asks for the shortest tour that visits each of $n$ cities exactly once and returns to the starting city. It is one of the most studied NP-hard optimization problems.

For SA applied to TSP, the natural neighbor function is the \emph{2-opt move}: reverse a segment of the tour. This operation uncrosses edges and typically reduces the tour length. A 2-opt move can be implemented in $O(n)$ time by reversing the segment between two randomly chosen positions.

\begin{cpplisting}
\textbf{C++ implementation:} The file \texttt{examples/example\_sa\_tsp.cpp} implements SA with 2-opt for TSP on 50 random cities, comparing against nearest-neighbor and random baseline tours.
\end{cpplisting}

%% ============================================================
\section{Application: The Knapsack Problem}
\label{sec:sa-knapsack}
%% ============================================================

The 0/1 knapsack problem: given $n$ items with weights $w_i$ and values $v_i$, select a subset that maximizes total value without exceeding capacity $C$. For SA, the neighbor function flips a single item in or out of the knapsack, and the cost function is the negated total value with a penalty for exceeding the capacity.

\begin{cpplisting}
\textbf{C++ implementation:} The file \texttt{examples/example\_sa\_knapsack.cpp} implements SA for a 100-item knapsack instance, demonstrating the improvement over a greedy baseline.
\end{cpplisting}
...
Simulated annealing was independently proposed by Kirkpatrick, Gelatt, and Vecchi (1983) and by \v{C}ern\'{y} (1985). The theoretical foundation connecting SA to Markov chain convergence was developed by Hajek (1988), who proved the logarithmic cooling schedule result. The metaphor of annealing in metallurgy was central to the original paper by Kirkpatrick et al., who noted that slow cooling of metals produces defect-free crystals---analogous to finding global optima.

The 2-opt neighborhood for TSP was introduced by Lin (1965) in the context of local search. The combination of 2-opt with simulated annealing was among the earliest applications of SA and remains competitive with more sophisticated metaheuristics for TSP.

\begin{itemize}[nosep]
  \item S.~Kirkpatrick, C.~D.~Gelatt, and M.~P.~Vecchi, ``Optimization by Simulated Annealing,'' \textit{Science}, 220(4598):671--680, 1983.
  \item V.~\v{C}ern\'{y}, ``Thermodynamical Approach to the Traveling Salesman Problem,'' \textit{J.~Optimization Theory and Applications}, 45(1):41--51, 1985.
  \item B.~Hajek, ``Cooling Schedules for Optimal Annealing,'' \textit{Mathematics of Operations Research}, 13(2):311--329, 1988.
  \item S.~Lin, ``Computer Solutions of the Traveling Salesman Problem,'' \textit{Bell System Technical Journal}, 44(10):2245--2269, 1965.
\end{itemize}

%==========================================================================
\chapter{Differential Privacy}
\label{ch:differential-privacy}
%==========================================================================

As organizations collect ever-larger datasets containing personal information---medical records, location traces, search histories---the question of how to extract useful statistics while protecting individual privacy has become urgent. \emph{Differential privacy} (DP), formalized by Dwork, McSherry, Nissim, and Smith (2006), provides a rigorous mathematical framework for privacy-preserving data analysis. The central idea is to add calibrated random noise to query results so that the presence or absence of any single individual has a negligible effect on the output.

%% ============================================================
\section{The Differential Privacy Framework}
\label{sec:dp-framework}
%% ============================================================

\begin{definition}[Neighboring databases]
Two databases $D$ and $D'$ are \emph{neighboring} if they differ in exactly one record (i.e., one record is added or removed).
\end{definition}

\begin{definition}[$\varepsilon$-Differential Privacy]
A randomized mechanism $\mathcal{M} : \mathcal{D} \to \mathcal{R}$ satisfies \emph{$\varepsilon$-differential privacy} if for all neighboring databases $D, D'$ and all subsets $S \subseteq \mathcal{R}$,
\[
\Pr[\mathcal{M}(D) \in S] \leq e^{\varepsilon} \cdot \Pr[\mathcal{M}(D') \in S].
\]
\end{definition}

The privacy parameter $\varepsilon$ controls the privacy-utility trade-off: smaller $\varepsilon$ means stronger privacy (the output distribution changes less when one individual is added or removed) but more noise and less utility. Typical values of $\varepsilon$ range from 0.1 (strong privacy) to 10 (weak privacy).

\subsection{Composition and Basic Properties}

Differential privacy has several elegant properties:

\begin{itemize}[nosep]
  \item \textbf{Sequential composition:} If $\mathcal{M}_1$ satisfies $\varepsilon_1$-DP and $\mathcal{M}_2$ satisfies $\varepsilon_2$-DP, then running both on the same database satisfies $(\varepsilon_1 + \varepsilon_2)$-DP.
  \item \textbf{Parallel composition:} If $\mathcal{M}_1$ and $\mathcal{M}_2$ run on disjoint subsets of the database, the combined mechanism satisfies $\max(\varepsilon_1, \varepsilon_2)$-DP.
  \item \textbf{Post-processing immunity:} Any function applied to the output of an $\varepsilon$-DP mechanism is also $\varepsilon$-DP.
\end{itemize}
...
The most fundamental DP mechanism adds Laplace noise to a numeric query.

\begin{definition}[Global sensitivity]
The \emph{global sensitivity} of a query $f : \mathcal{D} \to \mathbb{R}$ is $\Delta_f = \max_{D, D' \text{ neighboring}} |f(D) - f(D')|$.
\end{definition}

\begin{theorem}[Laplace Mechanism]\label{thm:laplace}
...
\begin{proof}
For neighboring databases $D$ and $D'$, the outputs $\mathcal{M}(D)$ and $\mathcal{M}(D')$ differ only in the location parameter of the Laplace distribution. The ratio of their probability density functions at any point $y$ is:
\[
\frac{p_{\mathcal{M}(D)}(y)}{p_{\mathcal{M}(D')}(y)} = \frac{(\varepsilon/\Delta_f) \exp(-\varepsilon |y - f(D)| / \Delta_f)}{(\varepsilon/\Delta_f) \exp(-\varepsilon |y - f(D')| / \Delta_f)} = \exp\!\left(\frac{\varepsilon}{\Delta_f} \bigl(|y - f(D')| - |y - f(D)|\bigr)\right).
\]
By the triangle inequality, $|y - f(D')| - |y - f(D)| \leq |f(D) - f(D')| \leq \Delta_f$, so the ratio is at most $e^{\varepsilon}$.
\end{proof}

The Laplace mechanism is the workhorse of differential privacy. It applies to any query whose sensitivity is bounded: sums, counts, means (with bounded inputs), histograms, and linear functions.

%% ============================================================
\section{Applications}
\label{sec:dp-applications}
%% ============================================================

\subsection{Private Mean Computation}

To release the mean of a bounded dataset $\{x_1, \ldots, x_n\}$ with values in $[0, B]$, compute the sum $S = \sum x_i$, add Laplace noise $\text{Lap}(2B/\varepsilon)$ (since the sensitivity of the sum is $2B$: one value can change from $0$ to $B$), and divide by $n$. The resulting noisy mean satisfies $\varepsilon$-DP, and its expected error is $O(B / (\varepsilon n))$---the error decreases with dataset size, a phenomenon known as \emph{privacy amplification by aggregation}.

\subsection{Private Histograms}

To release a histogram (frequency counts per bucket), split the privacy budget: allocate $\varepsilon/k$ to each of $k$ buckets, and add independent $\text{Lap}(k/\varepsilon)$ noise to each count. This satisfies $\varepsilon$-DP by sequential composition. The trade-off is clear: more buckets or smaller $\varepsilon$ means noisier per-bucket estimates.

\subsection{Private Count Queries}

A count query ``how many records satisfy property $P$?'' has sensitivity 1 (adding or removing one record changes the count by at most 1). The Laplace mechanism adds $\text{Lap}(1/\varepsilon)$ noise to the exact count, giving $\varepsilon$-DP with expected absolute error $1/\varepsilon$.

\begin{cpplisting}
\textbf{C++ implementation:} The files \texttt{examples/example\_laplace\_mechanism.cpp}, \texttt{examples/example\_private\_mean.cpp}, and \texttt{examples/example\_private\_histogram.cpp} demonstrate the Laplace mechanism, private mean computation, and private histograms with varying privacy budgets.
\end{cpplisting}
...
Differential privacy was formalized by Dwork, McSherry, Nissim, and Smith (2006), building on earlier work by Dwork et al. on random noise addition for privacy. The Laplace mechanism was part of the original framework. The conceptual breakthrough was defining privacy as a property of the \emph{randomized mechanism}, not of the data or the adversary.

The tension between privacy and utility is fundamental: perfectly private mechanisms (pure randomization) provide no utility, while perfectly useful mechanisms (no noise) provide no privacy. Differential privacy quantifies this trade-off through the parameter $\varepsilon$.

The choice of $\varepsilon$ is both technical and ethical. Technically, smaller $\varepsilon$ gives stronger privacy guarantees. Ethically, the appropriate value depends on the sensitivity of the data and the harms from privacy violations. The U.S. Census Bureau used $\varepsilon \approx 19.61$ for the 2020 Census, while Apple uses $\varepsilon \leq 8$ for some iOS features.

\begin{itemize}[nosep]
  \item C.~Dwork, F.~McSherry, K.~Nissim, and A.~Smith, ``Calibrating Noise to Sensitivity in Private Data Analysis,'' \textit{Proc.~TCC}, 265--284, 2006.
  \item C.~Dwork, ``Differential Privacy,'' \textit{Proc.~ICALP}, 1--12, 2006.
  \item C.~Dwork and A.~Roth, ``The Algorithmic Foundations of Differential Privacy,'' \textit{Foundations and Trends in Theoretical Computer Science}, 9(3-4):211--407, 2014.
  \item G.~Cormode, M.~Procopiuc, D.~Srivastava, and T.~X.~Tran, ``Differentially Private Spatial Decompositions,'' \textit{Proc.~ICDE}, 20--31, 2012.
\end{itemize}
...
%==========================================================================
\input{supplements/stage2_probability_toolkit.tex}
\input{supplements/stage3_classical_randomized_algorithms.tex}
\input{supplements/stage4_hashing_streaming.tex}
\input{supplements/stage5_random_graphs_derandomization.tex}
\input{supplements/stage6_markov_sampling.tex}
\input{supplements/stage7_graph_geometry_counting.tex}
...
%==========================================================================
\chapter{Industry Applications of Randomized Algorithms}
\label{app:applications}
\addcontentsline{toc}{chapter}{E Industry Applications of Randomized Algorithms}
\markboth{E Industry Applications of Randomized Algorithms}{Randomized Algorithms}
%==========================================================================

This appendix documents where and how the algorithms and data structures covered in 
this book are deployed in commercial software systems, databases, cloud architecture, 
and artificial intelligence applications.

%==========================================================================
\section{Graph \& Network Algorithms}
%==========================================================================

\subsection{Karger-Stein Min-Cut}
Used in network reliability analysis to identify weak points in routing topologies 
and electrical grids. In chip design (VLSI), min-cut partitioning is used to divide 
circuits across multiple physical chips or areas to minimize wire connections.

\subsection{Karger-Klein-Tarjan MST}
Used in theoretical modeling of high-speed optical fiber network designs and minimum-cost 
transportation networks. In practice, standard Kruskal's or Prim's algorithms are 
often preferred due to simpler cache layouts, but KKT remains a reference model for 
linear-time graph reduction.

\subsection{Seidel's APSP}
Utilized in social network analysis (estimating degrees of separation or closeness 
centrality in massive unweighted graphs) and routing systems that operate over 
unweighted, homogeneous networks (such as ad-hoc sensor grids).

\subsection{Luby's Parallel MIS}
Crucial for wireless communication networks to solve the channel assignment problem 
without central coordination. In distributed environments, it allows nodes to 
elect local group coordinators or allocate interference-free frequencies in parallel.
...
A cornerstone of modern high-performance databases. Skip Lists are deployed in:
\begin{itemize}
    \item \textbf{Redis}: Used to implement Sorted Sets (\texttt{ZSET}), enabling $O(\log N)$ insertions and range queries.
    \item \textbf{LevelDB / RocksDB}: Used as the in-memory write buffer (\texttt{MemTable}), because Skip Lists allow lock-free, concurrent insertions, performing significantly better than red-black trees under multi-threaded write loads.
\end{itemize}

\subsection{Treaps}
Used in wireless sensor network routing tables and geometric search applications. Their 
dual nature (key-based BST + random priority heap) makes them easy to split and merge, 
which is useful in range-query engines and segment intersection checks.

\subsection{Universal Hashing}
The standard defense mechanism against Hash DoS (Denial of Service) attacks. Programming 
language runtimes (Python, Rust, Java, C++) use randomized universal hashing families 
so that attackers cannot craft specific inputs to trigger $O(N)$ collisions in hash tables.

\subsection{FKS Perfect Hashing}
Used in hardware routers for lookup tables (IP lookup tables or firewall rules) where 
worst-case $O(1)$ lookup guarantees are required in hardware circuits, and memory is 
pre-allocated once.

%==========================================================================
\section{Algebraic \& String Algorithms}
%==========================================================================

\subsection{Freivalds' Verification}
Used in zero-knowledge proofs (ZKPs) and decentralized blockchain computation. It allows 
a light client to verify that a heavy cloud provider has computed a massive matrix 
multiplication correctly, in $O(N^2)$ time instead of recomputing it in $O(N^3)$ time.

\subsection{Rabin-Karp String Matching}
Widely used in plagiarism detection software and text document similarity checks. By 
calculating sliding fingerprints (rolling hashes), it can search for multiple patterns 
simultaneously, making it perfect for detecting copied paragraphs.

%==========================================================================
\section{Geometric \& LP Algorithms}
%==========================================================================

\subsection{RIC Convex Hull \& Delaunay Triangulation}
Foundational in GIS (Geographic Information Systems) for constructing Terrain Models 
(TINs) and in game engines for pathfinding mesh generation (navigation meshes). The 
randomized incremental approach prevents degenerate worst-case sorting arrangements.

\subsection{Seidel's LP Solver}
Used in computer graphics for real-time collision detection and boundary constraints 
solving. In embedded systems, it provides a lightweight, linear-time optimization 
solver for low-dimensional resource allocation without requiring heavy mathematical software.

%==========================================================================
\section{Probability \& Selection}
%==========================================================================

\subsection{LazySelect}
Utilized in distributed database engines to estimate quantiles and medians over 
sharded datasets. By sampling a subset, it reduces network data transfer by isolating 
the target range before running full database partitions.

\subsection{Online Paging}
Implemented in memory management units (MMUs), CPU caches, and content delivery networks 
(CDNs) like Cloudflare to evict redundant cached pages, balancing competitive miss 
ratios with low execution overhead.

%==========================================================================
\section{Modern Big-Data Sketches}
%==========================================================================

\subsection{HyperLogLog}
Deployed in \textbf{Google BigQuery, Elasticsearch, Amazon Redshift, and Redis}. It is used to 
estimate the number of unique active users (UAUs) over massive logs. HLL calculates 
billions of distinct IDs using a constant $12$ KB footprint instead of gigabytes of RAM.

\subsection{Count-Min Sketch}
Used in network traffic monitors (such as routers from Cisco or Juniper) to detect 
DDoS attacks by tracking heavy-hitter IP addresses in fast hardware memory. It is also 
used by Twitter/X to detect trending keywords and topics in real-time streams.

\subsection{Johnson-Lindenstrauss Random Projection}
A core component of \textbf{Vector Databases (Pinecone, Milvus, Qdrant)} and recommendation 
systems. It compresses 1000+ dimensional deep learning embeddings to 100 dimensions 
while preserving relative cosine distances, accelerating AI vector searches by up to 100x.

%==========================================================================
\chapter{Modern Relevancy and Frontiers of Randomized Algorithms}
\label{app:relevancy}
\addcontentsline{toc}{chapter}{F Modern Relevancy and Frontiers of Randomized Algorithms}
\markboth{F Modern Relevancy and Frontiers of Randomized Algorithms}{Randomized Algorithms}
%==========================================================================

This appendix provides a critical evaluation of the practical relevance of the 
algorithms covered in this book for modern software engineering, alongside an overview 
of newer randomized algorithms that represent active areas of industry development.

%==========================================================================
\section{Relevancy Status Matrix}
%==========================================================================

The table below catalogs the algorithms covered in this book and ranks their 
practical deployment status in modern industry:

\begin{table}[h]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Algorithm / Structure} & \textbf{Relevancy} & \textbf{Primary} \\
 & & \textbf{Domain} \\ \hline
Universal Hashing & High & Programming Languages, \\
 & & Hash Tables \\ \hline
Skip Lists & High & Databases (MemTables, Redis) \\ \hline
HyperLogLog & High & Big Data Analytics \\
 & & (BigQuery, Redshift) \\ \hline
Count-Min Sketch & High & Network Routing, \\
 & & Streaming Analytics \\ \hline
Johnson-Lindenstrauss & High & Vector Databases, \\
Projection & & AI Embeddings \\ \hline
Rabin-Karp Search & High & Plagiarism Detection, \\
 & & String Search \\ \hline
Randomized SVD & High & Machine Learning \\
 & & (scikit-learn PCA) \\ \hline
Treaps & Medium & Coordinate range \\
 & & search engines \\ \hline
RIC Convex Hull & Medium & GIS, Computational \\
 & & Geometry \\ \hline
Karger-Stein Min-Cut & Low (Theoretical) & Graph partitioning models \\ \hline
KKT MST & Low (Theoretical) & Expected linear MST studies \\ \hline
Seidel's LP Solver & Low (Theoretical) & Fixed-dimension LP research \\ \hline
\end{tabular}
\caption{Practical relevancy status of the textbook's algorithms in modern industry.}
\label{tab:relevancy-matrix}
\end{table}

%==========================================================================
\section{Analysis of Relevancy}
%==========================================================================

\subsection{Why Some Classic Algorithms Have Low Practical Relevancy}
\begin{itemize}
    \item \textbf{KKT MST}: Although theoretically $O(M)$ expected linear time, it is rarely implemented in practice because the constant factor is massive compared to Kruskal's or Prim's algorithms, which run in $O(M \log N)$ but benefit from extremely simple, cache-friendly array layouts.
    \item \textbf{Seidel's LP Solver}: Seidel's algorithm assumes the dimension $d$ is fixed and small. In real-world industry (operations research, logistics), linear programs have thousands of variables ($d > 10,000$). In these environments, deterministic simplex or interior-point methods are vastly superior.
\end{itemize}

\subsection{Why Modern Sketches Have High Relevancy}
Streaming and dimensionality reduction algorithms are the cornerstones of modern cloud scale. When datasets grow to petabytes, deterministic structures (like exact sets and full matrices) become too expensive to store in RAM. Approximating answers using bounded-error randomized algorithms is often the only viable engineering choice.

%==========================================================================
\section{Newer and Missing Algorithmic Frontiers}
%==========================================================================

The following randomized algorithms are widely deployed in the software industry today but represent newer developments that lie beyond the classical textbook topics:

\subsection{Bloom Filters and Cuckoo Filters (Membership Testing)}
Used in databases (like \textbf{Apache Cassandra, RocksDB, Google Bigtable}) to avoid expensive disk lookups. 
\begin{itemize}
    \item \textbf{How it works}: Uses a bit array and multiple random hash functions to check if an element is a member of a set. It returns either ``definitely not in set'' or ``possibly in set'' with a tiny false positive rate.
    \item \textbf{Cuckoo Filters}: A newer alternative (2014) that supports dynamic deletion of items by utilizing cuckoo hashing principles, replacing classic Bloom filters in modern index designs.
\end{itemize}

\subsection{Locality-Sensitive Hashing - LSH (Nearest Neighbors)}
Popularized for high-speed recommendation systems and duplicate web page clustering.
\begin{itemize}
    \item \textbf{How it works}: Unlike standard hashing which minimizes collisions, LSH uses randomized hash functions designed to *maximize* collisions for similar items, allowing sub-linear time nearest neighbor searches in high dimensions.
\end{itemize}

\subsection{Count-Sketch (Frequency with Sign)}
Used in advanced streaming queries where data elements can be subtracted (e.g., items leaving a queue).
\begin{itemize}
    \item \textbf{How it works}: It generalizes the Count-Min sketch by introducing a second random hash function that maps elements to $\pm 1$, keeping the frequency estimator unbiased and permitting negative updates.
\end{itemize}

\subsection{Markov Chain Monte Carlo - MCMC (Bayesian Inference)}
Crucial in modern statistical modeling, physics simulations, and generative AI.
\begin{itemize}
    \item \textbf{How it works}: Uses algorithms like \textbf{Metropolis-Hastings} and \textbf{Gibbs Sampling} to generate random samples from complex, high-dimensional probability distributions where direct sampling is mathematically impossible.
\end{itemize}

\subsection{Randomized Distributed Consensus (Ben-Or's Protocol)}
The backbone of fault-tolerant distributed networks and blockchain protocols.
\begin{itemize}
    \item \textbf{How it works}: When multiple nodes in a network need to agree on a state (e.g., a transaction ledger) in the presence of adversarial faults, they use randomized coin-tossing steps to break deadlocks and guarantee fast agreement with high probability.
\end{itemize}
